...
class LinkPreview:
    ...
    async def extract_link_heads(self, links: Links, config: CrawlerRunConfig) -> Links:
        ...
        self._log(
            "info",
            "Extracting head content for {count} filtered links",
        ...
        self._log(
            "info",
            "Completed head extraction for links, {success} successful",
            params={
                "success": len([r for r in head_results if r.get("status") == "valid"])
            },
        )
    ...
    def _filter_links(self, links: Links, link_config: Dict[str, Any]) -> List[str]:
        ...
        if include_patterns:
            ...
            self._log(
                "debug",
                "After include patterns: {count} links remain",
                params={"count": len(filtered_urls)},
            )
        ...
        if exclude_patterns:
            ...
            self._log(
                "debug",
                "After exclude patterns: {count} links remain",
                params={"count": len(filtered_urls)},
            )
        ...
        if max_links > 0 and len(filtered_urls) > max_links:
            ...
            self._log(
                "debug", "Limited to {max_links} links", params={"max_links": max_links}
            )
        ...
        self._log(
            "debug",
            "Final filtered URLs: {count} unique links",
    ...
    async def _extract_heads_parallel(
        self, urls: List[str], link_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if verbose:
            self._log(
                "info",
                "Starting batch processing: {total} links with {concurrency} concurrent workers",
        ...
        if verbose:
            ...
            results = await self._extract_with_progress(
                urls, seeding_config, link_config
            )
        else:
    ...
    async def _extract_with_progress(
        ...
        self._log(
            "info",
            "Batch processing completed: {completed}/{total} processed, {successful} successful, {failed} failed",
            params={
                "completed": completed,
                "total": total_urls,
    ...
    def _merge_head_data(
        ...
        return Links(internal=updated_internal, external=updated_external)
