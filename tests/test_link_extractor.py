    ...
    async with AsyncWebCrawler() as crawler:
        for url in test_urls:
            ...
            try:
                ...
                print(
                    f"🔍 Debug - Link extraction config: {config.link_preview_config.to_dict() if config.link_preview_config else None}"
                )
                ...
                if result.success:
                    ...
                    if all_links:
                        ...
                        for i, link in enumerate(all_links[:3]):
                            ...
                            if (
                                hasattr(link, "intrinsic_score")
                                and link.intrinsic_score is not None
                            ):
                                if link.intrinsic_score == float("inf"):
                                    print("     Intrinsic Score: ∞ (scoring disabled)")
                                else:
                                    print(
                                        f"     Intrinsic Score: {link.intrinsic_score:.2f}/10.0"
                                    )
                            else:
                            ...
                            if (
                                hasattr(link, "contextual_score")
                                and link.contextual_score is not None
                            ):
                                print(
                                    f"     Contextual Score: {link.contextual_score:.3f}"
                                )
                            else:
                            ...
                            # Show total score
                            if (
                                hasattr(link, "total_score")
                                and link.total_score is not None
                            ):
                    ...
                    if links_with_head:
                        ...
                        for i, link in enumerate(links_with_head[:3]):
                            print(f"\n  {i+1}. {link.href}")
                            print(f"     Status: {link.head_extraction_status}")
                            ...
                            if (
                                ...
                                if link.intrinsic_score == float("inf"):
                                    print(
                                        "       • Intrinsic Score: ∞ (scoring disabled)"
                                    )
                                else:
                                    print(
                                        f"       • Intrinsic Score: {link.intrinsic_score:.2f}/10.0"
                    ...
                    failed_links = [
                        link
                        for link in internal_links + external_links
                        if hasattr(link, "head_extraction_status")
                        and link.head_extraction_status == "failed"
                    ]
                    ...
                    if failed_links:
                        ...
                        for link in failed_links[:2]:  # Show first 2 failures
                            print(f"  - {link.href}")
                            if (
                                hasattr(link, "head_extraction_error")
                                and link.head_extraction_error
                            ):
                                print(f"    Error: {link.head_extraction_error}")
