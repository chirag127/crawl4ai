...
def get_codebase_imports_with_files(
    root_dir: Path,
    ...
    target_dirs = [root_dir / "crawl4ai", root_dir / "docs"]
...
def check_missing_dependencies():
    ...
    for imp, file_info in external_imports.items():
        ...
        for declared in normalized_declared:
            if (
                normalized_imp == declared
                or normalized_imp.startswith(declared + ".")
                or declared.startswith(normalized_imp)
            ):
                found = True
                break
    ...
    if not_declared:
        print(
            "❌ External imports NOT declared in pyproject.toml or requirements.txt:\n"
        )
    ...
    for dep in declared_deps:
        ...
        for used in used_packages:
            if (
                ...
                or normalized_dep.startswith(used)
            ):
                found_usage = True
                break
