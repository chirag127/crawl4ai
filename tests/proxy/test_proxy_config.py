...
class TestProxyConfig:
    ...
    TEST_PROXY_DATA = {"server": "", "username": "", "password": "", "ip": ""}
    ...
    def test_proxy_config_special_characters_in_credentials(self):
        ...
        for username, password in special_chars_tests:
            proxy = ProxyConfig(
                server="http://127.0.0.1:8080", username=username, password=password
            )
    ...
    if proxies:
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            try:
                ...
                print(
                    result.markdown[:200] if result and result.markdown else "No result"
                )
            except Exception as e:
