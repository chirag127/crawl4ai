...
)  # If server is running in Docker, use the host's IP
BASE_URL = os.getenv(
    "CRAWL4AI_TEST_URL", "http://localhost:8020"
)  # If server is running in dev debug mode
...
async def assert_crawl_result_structure(result: Dict[str, Any], check_ssl=False):
    ...
    if check_ssl:
        ...
        assert (
            isinstance(result["ssl_certificate"], dict)
            or result["ssl_certificate"] is None
        )
        ...
        if line:
            try:
                ...
                if data.get("status") == "completed":
                    ...
                    print(
                        f"Received non-result JSON line: {data}"
                    )  # Log other status messages if needed
...
class TestDeepCrawlEndpoints:
    ...
    async def test_deep_crawl_with_scoring(self, async_client: httpx.AsyncClient):
        ...
        product_urls_found = any(
            "product_" in result["url"]
            for result in data["results"]
            if result["metadata"]["depth"] > 0
        )
    ...
    async def test_deep_crawl_with_css_extraction(
        ...
        for result in data["results"]:
            ...
            if "product_" in result["url"]:  # Check product pages specifically
                ...
                try:
                    ...
                    pytest.fail(
                        f"Extraction validation failed for {result['url']}: {e}\nContent: {result['extracted_content']}"
                    )
            # else:
        ...
        assert (
            found_extracted_product
        ), "Did not find any pages where product data was successfully extracted."

    # 5. Deep Crawl with LLM Extraction (Requires Server LLM Setup)
    async def test_deep_crawl_with_llm_extraction(
        self, async_client: httpx.AsyncClient
    ):
        ...
        try:
            ...
            pytest.fail(
                f"Deep Crawl + LLM extraction request failed: {e}. Response: {e.response.text}. Check server logs and LLM API key setup."
            )
        ...
        for result in data["results"]:
            ...
            try:
                ...
                if isinstance(extracted, list):
                    extracted = extracted[0]  # Handle list output
                ...
                print(
                    f"LLM extracted from {result['url']}: Title='{extracted.get('h1_title')}'"
                )
            except (json.JSONDecodeError, AssertionError, IndexError, TypeError) as e:
                pytest.fail(
                    f"LLM extraction validation failed for {result['url']}: {e}\nContent: {result['extracted_content']}"
                )

        assert (
            found_llm_extraction
        ), "LLM extraction did not yield expected data on any crawled page."
    ...
    async def test_deep_crawl_with_ssl(self, async_client: httpx.AsyncClient):
        ...
        await assert_crawl_result_structure(
            result, check_ssl=True
        )  # <-- Tell helper to check SSL field
        assert result["success"] is True
        ...
        if result["ssl_certificate"]:
            ...
            print(
                f"SSL Issuer Org: {result['ssl_certificate'].get('issuer', {}).get('O', 'N/A')}"
            )
            print(
                f"SSL Valid From: {result['ssl_certificate'].get('not_before', 'N/A')}"
            )
        else:
    ...
    async def test_deep_crawl_with_proxies(self, async_client: httpx.AsyncClient):
        ...
        if not proxies:
            pytest.skip(
                "Skipping proxy test: PROXIES environment variable not set or empty."
            )
                ...
                "params": {
                    ...
                    "proxy_rotation_strategy": {  # <-- Define the strategy
                        ...
                        "params": {
                            ...
                            "proxies": [
                                {"type": "ProxyConfig", "params": p} for p in proxies
                            ]
                        },
                    },
                    "deep_crawl_strategy": {
        ...
        try:
            ...
            pytest.fail(
                f"Proxy deep crawl failed: {e}. Response: {e.response.text}. Are proxies valid and accessible by the server?"
            )
        except httpx.RequestError as e:
            pytest.fail(
                f"Proxy deep crawl request failed: {e}. Are proxies valid and accessible?"
            )
        ...
        print(
            f"Proxy deep crawl completed successfully for {len(data['results'])} pages."
        )
