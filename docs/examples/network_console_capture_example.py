    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if result.success and result.network_requests:
            ...
            if response:
                ...
                print(
                    f"  Status: {response.get('status')} {response.get('status_text', '')}"
                )
...
async def demo_basic_console_capture():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        result = await crawler.arun(url=f"file://{html_file}", config=config)
...
async def demo_combined_capture():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if result.success:
            network_count = (
                len(result.network_requests) if result.network_requests else 0
            )
            console_count = (
                len(result.console_messages) if result.console_messages else 0
            )

            print(
                f"Captured {network_count} network events and {console_count} console messages"
            )
...
async def analyze_spa_network_traffic():
    ...
    async with AsyncWebCrawler(
        config=BrowserConfig(headless=True, viewport_width=1280, viewport_height=800)
    ) as crawler:
        config = CrawlerRunConfig(
            capture_network_requests=True,
            capture_console_messages=True,
        ...
        result = await crawler.arun(url="https://weather.com", config=config)
        ...
        if result.success and result.network_requests:
            ...
            print(
                f"Captured {len(requests)} requests, {len(responses)} responses, and {len(failures)} failures"
            )
            ...
            if failures:
                ...
                for i, failure in enumerate(failures[:5], 1):  # Show first 5
                    print(
                        f"  {i}. {failure.get('url')} - {failure.get('failure_text')}"
                    )
                if len(failures) > 5:
            ...
            if result.console_messages:
                errors = [
                    msg for msg in result.console_messages if msg.get("type") == "error"
                ]
                if errors:
            ...
            output_file = os.path.join(
                __cur_dir__, "tmp", "weather_network_analysis.json"
            )
            with open(output_file, "w") as f:
                json.dump(
                    {
                        ...
                        "statistics": {
                            ...
                            "status_codes": {
                                str(k): v for k, v in status_codes.items()
                            },
                            "api_call_count": len(api_calls),
                            "console_error_count": (
                                len(errors) if result.console_messages else 0
                            ),
                        },
                        "network_requests": result.network_requests,
                        "console_messages": result.console_messages,
...
async def demo_security_analysis():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        result = await crawler.arun(url="https://www.nytimes.com/", config=config)
        ...
        if result.success and result.network_requests:
            ...
            insecure_requests = [
                ...
                for req in result.network_requests
                if req.get("event_type") == "request"
                and req.get("url", "").startswith("http://")
            ]
...
async def demo_performance_analysis():
    ...
    async with AsyncWebCrawler() as crawler:
        config = CrawlerRunConfig(
            capture_network_requests=True, page_timeout=60 * 2 * 1000  # 120 seconds
        )

        result = await crawler.arun(url="https://www.cnn.com/", config=config)
        ...
        if result.success and result.network_requests:
            ...
            if responses_with_timing:
                print(
                    f"Analyzing timing for {len(responses_with_timing)} network responses"
                )
                ...
                for resp in responses_with_timing:
                    ...
                    if (
                        ...
                        and "requestTime" in timing
                        and "receiveHeadersEnd" in timing
                    ):
                        ...
                        resource_timings[resource_type].append(
                            {"url": url, "duration_ms": duration}
                        )
                    if (
                        isinstance(timing, dict)
                        and "requestStart" in timing
                        and "responseStart" in timing
                        and "startTime" in timing
                    ):
                        # Convert to milliseconds
                        duration = (
                ...
                for resource_type, timings in resource_timings.items():
                    if timings:
                        ...
                        slowest_resource = next(
                            t["url"]
                            for t in timings
                            if t["duration_ms"] == max_duration
                        )
                ...
                for i, timing in enumerate(all_timings[:5], 1):
                    print(
                        f"  {i}. [{timing['type']}] {timing['url']} - {timing['duration_ms']:.2f} ms"
                    )

                # Save performance analysis to file
                output_file = os.path.join(
                    __cur_dir__, "tmp", "performance_analysis.json"
                )
                with open(output_file, "w") as f:
                    json.dump(
                        {
...
async def main():
    ...
    print("\n=== Examples Complete ===")
    print(
        f"Check the tmp directory for output files: {os.path.join(__cur_dir__, 'tmp')}"
    )
