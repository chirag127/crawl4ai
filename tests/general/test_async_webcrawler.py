...
from crawl4ai import (AsyncWebCrawler, BrowserConfig, CacheMode,
                      CrawlerRunConfig, MemoryAdaptiveDispatcher, RateLimiter)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("viewport", [(800, 600), (1024, 768), (1920, 1080)])
...
async def test_memory_management():
    ...
    browser_config = BrowserConfig(
        browser_type="chromium", headless=True, viewport_width=1024, viewport_height=768
    )

    dispatcher = MemoryAdaptiveDispatcher(
        memory_threshold_percent=70.0, check_interval=1.0, max_session_permit=5
    )
...
async def test_rate_limiting():
    ...
    dispatcher = MemoryAdaptiveDispatcher(
        rate_limiter=RateLimiter(base_delay=(1.0, 2.0), max_delay=5.0, max_retries=2),
...
async def test_javascript_execution():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        result = await crawler.arun(
            url="https://example.com",
            config=CrawlerRunConfig(js_code=js_code, page_timeout=30000),
        )
        assert result.success
...
async def test_extraction_strategy_run_with_regular_url():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        assert (
            mock_strategy.run_calls[0] == regular_url
        ), f"Expected '{regular_url}', got '{mock_strategy.run_calls[0]}'"
...
async def test_extraction_strategy_run_with_raw_html():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        raw_html_url = (
            "raw:<html><body><h1>Test HTML</h1><p>This is a test.</p></body></html>"
        )
        result = await crawler.arun(
            url=raw_html_url,
            config=CrawlerRunConfig(
        ...
        assert len(mock_strategy.run_calls) == 1
        assert (
            mock_strategy.run_calls[0] == "Raw HTML"
        ), f"Expected 'Raw HTML', got '{mock_strategy.run_calls[0]}'"
