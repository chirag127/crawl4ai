...
from crawl4ai import (BM25ContentFilter, CacheMode, CrawlerRunConfig,
                      DefaultMarkdownGenerator, JsonCssExtractionStrategy,
                      RegexChunking)
from crawl4ai.deep_crawling import BFSDeepCrawlStrategy
from crawl4ai.deep_crawling.filters import (FastContentTypeFilter,
                                            FastDomainFilter, FastFilterChain)
...
def create_test_config() -> CrawlerRunConfig:
    ...
    url_scorer = FastKeywordRelevanceScorer(keywords=["article", "blog"], weight=1.0)
...
def test_config_serialization_cycle():
    ...
    assert (
        deserialized_config.word_count_threshold == original_config.word_count_threshold
    )
    ...
    assert isinstance(
        deserialized_config.deep_crawl_strategy.filter_chain, FastFilterChain
    )
    assert isinstance(
        deserialized_config.deep_crawl_strategy.url_scorer, FastKeywordRelevanceScorer
    )
