...
async def demo_1_adaptive_crawling():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        result = await adaptive.digest(
            start_url="https://docs.python.org/3/", query="python decorators tutorial"
        )
        ...
        console.print(
            f"  - Knowledge Base: {len(adaptive.state.knowledge_base)} documents"
        )
...
async def demo_2_virtual_scroll():
    ...
    console.print(
        "[dim]with actual infinite scroll pages like social media feeds.[/dim]\n"
    )
...
async def demo_4_url_seeder():
    ...
    print_section("Demo 4: URL Seeder", "Intelligent URL discovery and filtering")
...
async def demo_5_c4a_script():
    ...
    print_section(
        "Demo 5: C4A Script Language", "Domain-specific language for web automation"
    )
...
"""
    ...
    if compilation_result.success:
        ...
        console.print(
            f"  - Generated {len(compilation_result.js_code)} JavaScript statements"
        )
...
async def demo_6_css_extraction():
    ...
    print_section(
        "Demo 6: Enhanced Extraction", "Improved CSS selector and JSON extraction"
    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if result.success and result.extracted_content:
            ...
            console.print(
                f"Extracted: {json.dumps(json.loads(result.extracted_content), indent=2)[:200]}..."
            )
