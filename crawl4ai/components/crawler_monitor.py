...
class TerminalUI:
    ...
    def _create_status_panel(self) -> Panel:
        ...
        # Format runtime
        runtime = self.monitor._format_time(
            time.time() - self.monitor.start_time if self.monitor.start_time else 0
        )

        # Create the status text
        status_text = Text()
        status_text.append(
            f"Web Crawler Dashboard | Runtime: {runtime} | Memory: {memory_percent:.1f}% {memory_icon}\n"
        )
        status_text.append(
            f"Status: {memory_status} | URLs: {summary['urls_completed']}/{summary['urls_total']} | "
        )
        status_text.append(
            f"Peak Mem: {summary['peak_memory_percent']:.1f}% at {self.monitor._format_time(summary['peak_memory_time'])}"
        )
    ...
    def _create_pipeline_panel(self) -> Panel:
        ...
        table.add_row(
            "", "", "", "Requeue Rate", f"{summary.get('requeue_rate', 0):.1f}%"
        )
    ...
    def _create_task_details_panel(self) -> Panel:
        ...
        for task_id, stats in sorted(
            ...
            key=lambda x: (
                (
                    0
                    if x[1]["status"] == CrawlStatus.IN_PROGRESS.name
                    else 1 if x[1]["status"] == CrawlStatus.QUEUED.name else 2
                ),
    ...
    def _create_footer(self) -> Panel:
        ...
        status_style = (
            ...
            else "yellow" if memory_status == "PRESSURE" else "red bold"
        )
        ...
        footer_content = Columns(
            [Align.left(left_text), Align.center(center_text), Align.right(right_text)],
            expand=True,
        )
...
class CrawlerMonitor:
    ...
    def __init__(
        ...
        self.terminal_ui = (
            TerminalUI(refresh_rate=refresh_rate, max_width=max_width)
            if enable_ui
            else None
        )
    ...
    def get_summary(self) -> Dict:
        ...
        with self._lock:
            ...
            completed_tasks = [
                task
                for task in self.stats.values()
                if task["status"] == CrawlStatus.COMPLETED.name
            ...
            if completed_tasks:
                total_duration = sum(
                    task["end_time"] - task["start_time"] for task in completed_tasks
                )
            ...
            if (
                avg_task_duration > 0
                and self.urls_total > 0
                and self.urls_completed > 0
            ):
