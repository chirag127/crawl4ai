...
async def get_sponsors(active: Optional[bool] = True):
    ...
    if active:
        ...
        sponsors = [
            s
            for s in sponsors
            if (not s.get("start_date") or s["start_date"] <= now)
            and (not s.get("end_date") or s["end_date"] >= now)
        ]
...
async def upload_image(file: UploadFile = File(...), folder: str = Form("sponsors")):
    ...
    filename = (
        f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{secrets.token_hex(8)}{extension}"
    )
...
async def admin_login(password: str = Body(..., embed=True)):
    ...
    return {"token": token, "expires_in": Config.TOKEN_EXPIRY_HOURS * 3600}
...
async def get_admin_stats():
    ...
    stats = {
        ...
        "total_views": sum(
            app.get("views", 0) for app in db.get_all("apps", limit=10000)
        ),
    }
    return stats
...
async def create_sponsor(sponsor_data: Dict[str, Any]):
    ...
    try:
        ...
        cursor.execute(
            f"INSERT INTO sponsors ({columns}) VALUES ({placeholders})",
...
async def update_sponsor(sponsor_id: int, sponsor_data: Dict[str, Any]):
    ...
    try:
        ...
        cursor = db.conn.cursor()
        cursor.execute(
            f"UPDATE sponsors SET {set_clause} WHERE id = ?",
