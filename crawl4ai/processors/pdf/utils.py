...
def apply_png_predictor(data, width, bits, color_channels):
    ...
    for i in range(num_lines):
        ...
        if filter_type == 0:  # None
            ...
            decoded = bytearray(
                [(filtered[j] + prev_line[j]) % 256 for j in range(len(filtered))]
            )
...
def clean_pdf_text_to_html(page_number, text):
    ...
    def flush_paragraph():
        if current_paragraph:
            ...
            if para:
                ...
                output.append(
                    f'<div class="paragraph">{"".join(escaped_para)}</div><hr/>'
                )
    ...
    for i, line in enumerate(lines):
        ...
        if (
            not article_title_detected
            and i == 0
            and 3 <= len(line.split()) <= 8
            and len(lines) > 1
        ):
            flush_paragraph()
        ...
        if page_number == 1 and author_pattern.match(line):
            ...
            if len(formatted_authors) > 1:
                joined = (
                    ", ".join(formatted_authors[:-1]) + " and " + formatted_authors[-1]
                )
            else:
    ...
    html_output = re.sub(
        r"\(([A-Z][a-z]+ et al\. \d{4})\)", r"<cite>\1</cite>", html_output
    )
