    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if result.success:
            ...
            for i, table in enumerate(result.tables):
                ...
                print(
                    f"  Size: {table['metadata']['row_count']} rows × {table['metadata']['column_count']} columns"
                )
...
async def example_disable_extraction():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if result.success:
            ...
            print(
                "Table extraction disabled - better performance for non-table content"
            )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        config = CrawlerRunConfig(
            ...
            table_extraction=FinancialTableExtraction(
                currency_symbols=["$", "€"], verbose=True
            ),
        )

        result = await crawler.arun("https://finance.yahoo.com/", config=config)
        ...
        if result.success:
            ...
            for table in result.tables:
                if table["metadata"].get("type") == "financial":
                    print(
                        f"  ✓ Financial table with {table['metadata']['row_count']} rows"
                    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        config = CrawlerRunConfig(
            ...
            table_extraction=DefaultTableExtraction(
                table_score_threshold=6, min_rows=2
            ),
            # LLM extraction for structured data
            extraction_strategy=LLMExtractionStrategy(
                llm_config=LLMConfig(provider="openai"), schema=schema
            ),
        )

        result = await crawler.arun(
            "https://en.wikipedia.org/wiki/Economy_of_the_United_States", config=config
        )
