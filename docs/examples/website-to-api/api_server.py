...
def save_api_request(
    ...
    if (
        endpoint in ["/scrape", "/scrape-with-llm"]
        and "url" in body
        and "query" in body
    ):
        ...
        for existing_request in existing_requests:
            if (
                existing_request.endpoint == endpoint
                and existing_request.body.get("url") == body["url"]
                and existing_request.body.get("query") == body["query"]
            ):
                print(
                    f"Duplicate request found for URL: {body['url']} and query: {body['query']}"
                )
                return (
                    existing_request.id
...
async def scrape_website_endpoint(request: ScrapeRequest):
    ...
    try:
        ...
        result = await scraper_agent.scrape_data(
            url=str(request.url), query=request.query, model_name=request.model_name
        )
...
async def list_saved_requests():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list saved requests: {str(e)}"
        )
...
async def delete_saved_request(request_id: str):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            raise HTTPException(
                status_code=404, detail=f"Saved request '{request_id}' not found"
            )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to delete saved request: {str(e)}"
        )
...
async def list_cached_schemas():
    ...
    try:
        ...
        return {"success": True, "cached_schemas": schemas, "count": len(schemas)}
    except Exception as e:
...
async def save_model_config(request: ModelConfigRequest):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to save model configuration"
            )
...
async def delete_model_config(model_name: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail=f"Model configuration '{model_name}' not found"
            )
