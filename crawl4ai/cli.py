...
from crawl4ai import (AsyncWebCrawler, BestFirstCrawlingStrategy,
                      BFSDeepCrawlStrategy, BM25ContentFilter, BrowserConfig,
                      BrowserProfiler, CacheMode, CrawlerRunConfig,
                      CrawlResult, DefaultMarkdownGenerator,
                      DFSDeepCrawlStrategy, JsonCssExtractionStrategy,
                      JsonXPathExtractionStrategy, LLMConfig,
                      LLMExtractionStrategy, LXMLWebScrapingStrategy,
...
def setup_llm_config() -> tuple[str, str]:
    ...
    if not config.get("DEFAULT_LLM_PROVIDER") or not config.get(
...
async def stream_llm_response(
    url: str, markdown: str, query: str, provider: str, token: str
):
    response = completion(
        model=provider,
        api_key=token,
...
async def run_crawler(
    url: str, browser_cfg: BrowserConfig, crawler_cfg: CrawlerRunConfig, verbose: bool
):
    if verbose:
...
def display_profiles_table(profiles: List[Dict[str, Any]]):
    ...
    if not profiles:
        console.print(
            Panel(
                "[yellow]No profiles found. Create one with the 'create' command.[/yellow]",
...
async def create_profile_interactive(profiler: BrowserProfiler):
    ...
    console.print(
        Panel(
            "[bold cyan]Create Browser Profile[/bold cyan]\n"
    ...
    profile_name = Prompt.ask(
        "[cyan]Enter profile name[/cyan]", default=f"profile_{int(time.time())}"
    )

    console.print("[cyan]Creating profile...[/cyan]")
    console.print(
        "[yellow]A browser window will open. After logging in to sites, press 'q' in this terminal to save.[/yellow]"
    )
    ...
    try:
        ...
        if profile_path:
            console.print(
                f"[green]Profile successfully created at:[/green] {profile_path}"
            )
        else:
...
def delete_profile_interactive(profiler: BrowserProfiler):
    ...
    try:
        ...
        # Confirm deletion
        if Confirm.ask(
            f"[red]Are you sure you want to delete profile '{profile['name']}'?[/red]"
        ):
            ...
            if success:
                ...
                console.print(
                    f"[red]Failed to delete profile '{profile['name']}'.[/red]"
                )
...
async def crawl_with_profile_cli(profile_path, url):
    ...
    console.print(
        f"[cyan]Crawling [bold]{url}[/bold] using profile at [bold]{profile_path}[/bold][/cyan]"
    )
    ...
    try:
        ...
        config = get_global_config()
        ensure_ascii = config.get(
            "JSON_ENSURE_ASCII", USER_SETTINGS["JSON_ENSURE_ASCII"]["default"]
        )

        # Handle output
        if output_format == "all":
            console.print(
                json.dumps(result.model_dump(), indent=2, ensure_ascii=ensure_ascii)
            )
...
async def manage_profiles():
    ...
    while True:
        console.print(
            Panel("[bold cyan]Browser Profile Manager[/bold cyan]", border_style="cyan")
        )
        ...
        for key, value in options.items():
            color = (
                ...
                if key == "1"
                    ...
                    "yellow"
                    if key == "2"
                    else "red" if key == "3" else "blue" if key == "4" else "cyan"
                )
            )
...
@click.option(
    "--browser-type",
    "-b",
    type=click.Choice(["chromium", "firefox"]),
    default="chromium",
...
def browser_start_cmd(browser_type: str, port: int, headless: bool):
    ...
    try:
        ...
        cdp_url = anyio.run(
            profiler.launch_builtin_browser, browser_type, port, headless
        )
...
def browser_view_cmd(url: Optional[str]):
    ...
    try:
        ...
        try:
            ...
            console.print(
                "[green]Browser window opened. Close it when finished viewing.[/green]"
            )
        except Exception as e:
            console.print(f"[red]Error launching browser: {str(e)}[/red]")
            console.print(
                f"[yellow]Try connecting manually to {cdp_url} in Chrome or using the '--remote-debugging-port' flag.[/yellow]"
            )
...
@click.option(
    ...
    type=int,
    default=None,
    help="Debugging port (defaults to same as current)",
)
@click.option(
    "--headless/--no-headless", default=None, help="Run browser in headless mode"
)
def browser_restart_cmd(
    browser_type: Optional[str], port: Optional[int], headless: Optional[bool]
):
    ...
    try:
        ...
        headless = (
            headless if headless is not None else current_config.get("headless", True)
        )
...
@click.option(
    "--user-data-dir",
    "-d",
    help="Directory to use for browser data (will be created if it doesn't exist)",
)
@click.option(
    "--port", "-P", type=int, default=9222, help="Debugging port (default: 9222)"
)
...
@click.option(
    "--incognito",
    is_flag=True,
    help="Run in incognito/private mode (ignores user-data-dir)",
)
def cdp_cmd(
    ...
    try:
        ...
        if not incognito and user_data_dir:
            ...
            if not os.path.exists(expanded_path):
                console.print(
                    f"[yellow]Directory '{expanded_path}' doesn't exist. Creating it.[/yellow]"
                )
        ...
        # Run the browser
        cdp_url = anyio.run(
            profiler.launch_standalone_browser, browser_type, data_dir, port, headless
        )
...
@click.option(
    ...
    help="Browser config file (YAML/JSON)",
)
@click.option(
    "--crawler-config",
    "-C",
    type=click.Path(exists=True),
    help="Crawler config file (YAML/JSON)",
)
@click.option(
    "--filter-config",
    "-f",
    type=click.Path(exists=True),
    help="Content filter config file",
)
@click.option(
    "--extraction-config",
    "-e",
    type=click.Path(exists=True),
    help="Extraction strategy config file",
)
@click.option(
    "--json-extract",
    "-j",
    ...
    flag_value="",
    default=None,
    help="Extract structured data using LLM with optional description",
)
@click.option(
    "--schema", "-s", type=click.Path(exists=True), help="JSON schema for extraction"
)
@click.option(
    "--browser",
    "-b",
    ...
    help="Browser parameters as key1=value1,key2=value2",
)
@click.option(
    "--crawler",
    "-c",
    type=str,
    callback=parse_key_values,
    help="Crawler parameters as key1=value1,key2=value2",
)
@click.option(
    "--output",
    "-o",
    type=click.Choice(["all", "json", "markdown", "md", "markdown-fit", "md-fit"]),
    default="all",
)
@click.option(
    "--output-file", "-O", type=click.Path(), help="Output file path (default: stdout)"
)
@click.option(
    "--bypass-cache",
    "-bc",
...
@click.option(
    "--deep-crawl",
    type=click.Choice(["bfs", "dfs", "best-first"]),
    help="Enable deep crawling with specified strategy (bfs, dfs, or best-first)",
)
@click.option(
    "--max-pages",
    type=int,
    default=10,
    help="Maximum number of pages to crawl in deep crawl mode",
)
@click.option(
    "--json-ensure-ascii/--no-json-ensure-ascii",
    default=None,
    help="Escape non-ASCII characters in JSON output (default: from global config)",
)
def crawl_cmd(
    url: str,
    browser_config: str,
    ...
    if profile:
        ...
        if not profile_path:
            ...
            if profiles:
                console.print(
                    f"[red]Profile '{profile}' not found. Available profiles:[/red]"
                )
                display_profiles_table(profiles)
            else:
                console.print(
                    "[red]No profiles found. Create one with 'crwl profiles'[/red]"
                )
    ...
    try:
        ...
        if filter_config or output in ["markdown-fit", "md-fit"]:
            if filter_config:
                ...
                filter_conf = {"type": "pruning", "query": "", "threshold": 0.48}
...
Always return valid, properly formatted JSON."""
            ...
            default_instruction_with_user_query = (
            ...
            crawler_cfg.extraction_strategy = LLMExtractionStrategy(
                ...
                schema=load_schema_file(
                    schema
                ),  # Will be None if no schema is provided
        ...
        # Handle extraction strategy from config file (only if json-extract wasn't used)
            ...
            if extract_conf["type"] not in ["llm", "json-css", "json-xpath"]:
                raise click.ClickException(
                    f"Invalid extraction type: {extract_conf['type']}"
                )

            if extract_conf["type"] == "llm":
                # if no provider show error emssage
                if not extract_conf.get("provider") or not extract_conf.get(
                    "api_token"
                ):
                    raise click.ClickException(
                        "LLM provider and API token are required for LLM extraction"
                    )

                crawler_cfg.extraction_strategy = LLMExtractionStrategy(
                    llm_config=LLMConfig(
                        provider=extract_conf["provider"],
                        api_token=extract_conf["api_token"],
                    ),
        ...
        if deep_crawl:
            if deep_crawl == "bfs":
                crawler_cfg.deep_crawl_strategy = BFSDeepCrawlStrategy(
                    max_depth=3, max_pages=max_pages
                )
            elif deep_crawl == "dfs":
            ...
            if verbose:
                console.print(
                    f"[green]Deep crawling enabled:[/green] {deep_crawl} strategy, max {max_pages} pages"
                )
        ...
        if not output_file:
            if output == "all":
                if isinstance(result, list):
                    ...
                    click.echo(
                        json.dumps(output_data, indent=2, ensure_ascii=ensure_ascii)
                    )
                else:
                    click.echo(
                        json.dumps(
                            main_result.model_dump(),
                            indent=2,
                            ensure_ascii=ensure_ascii,
                        )
                    )
                ...
                click.echo(
                    json.dumps(extracted_items, indent=2, ensure_ascii=ensure_ascii)
                )
...
def config_list_cmd():
    ...
    for key, setting in USER_SETTINGS.items():
        ...
        table.add_row(key, str(display_value), default_value, setting["description"])
...
def config_set_cmd(key: str, value: str):
    ...
    if key not in USER_SETTINGS:
        console.print(f"[red]Error: Unknown setting '{key}'[/red]")
        console.print(
            f"[yellow]Available settings: {', '.join(USER_SETTINGS.keys())}[/yellow]"
        )
        return
    ...
    if setting["type"] == "boolean":
        if value.lower() in ["true", "yes", "1", "y"]:
            ...
            console.print(
                "[red]Error: Invalid boolean value. Use 'true' or 'false'.[/red]"
            )
            return
        ...
        if "options" in setting and value not in setting["options"]:
            console.print(
                f"[red]Error: Value must be one of: {', '.join(setting['options'])}[/red]"
            )
            return
    ...
    console.print(
        f"[green]Successfully set[/green] [cyan]{key}[/cyan] = [green]{display_value}[/green]"
    )
...
def profiles_create_cmd(name: str):
    ...
    console.print(
        Panel(
            f"[bold cyan]Creating Profile: {name}[/bold cyan]\n"
...
@click.option(
    "--bypass-cache", is_flag=True, default=True, help="Bypass cache when crawling"
)
@click.option("--question", "-q", help="Ask a question about the crawled content")
@click.option("--verbose", "-v", is_flag=True)
@click.option("--profile", "-p", help="Use a specific browser profile (by name)")
@click.option(
    "--deep-crawl",
    type=click.Choice(["bfs", "dfs", "best-first"]),
    help="Enable deep crawling with specified strategy",
)
@click.option(
    "--max-pages",
    type=int,
