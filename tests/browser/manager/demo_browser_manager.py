...
async def basic_pooling_demo():
    ...
    config1 = BrowserConfig(
        browser_type="chromium", headless=True, browser_mode="playwright"
    )

    config2 = BrowserConfig(browser_type="chromium", headless=True, browser_mode="cdp")
    ...
    try:
        ...
        await manager.initialize_pool(
            browser_configs=[config1, config2], browsers_per_config=2
        )
        ...
        async def simulate_crawl(
            index: int, config: BrowserConfig, run_config: CrawlerRunConfig
        ):
...
async def prewarm_pages_demo():
    ...
    try:
        ...
        await manager.initialize_pool(
            browser_configs=[config], browsers_per_config=2, page_configs=page_configs
        )
...
async def prewarm_on_demand_demo():
    ...
    page_configs = [(config, run_config, 2)]
...
async def high_volume_demo():
    ...
    page_configs = [(config, run_config, total_pages)]

    print(
        f"Preparing {browser_count} browsers with {pages_per_browser} pages each ({total_pages} total pages)"
    )
