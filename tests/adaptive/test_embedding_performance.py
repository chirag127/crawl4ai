...
async def create_mock_crawl_results(n: int) -> list:
    ...
    for i in range(n):
        ...
        content = (
            f"This is test content {i} about async await coroutines event loops. " * 50
        )
...
async def test_embedding_performance():
    ...
    strategy = EmbeddingStrategy(
        embedding_model=config.embedding_model, llm_config=config.embedding_llm_config
    )
    ...
    query_embeddings, expanded_queries = await strategy.map_query_semantic_space(
        state.query, config.n_query_variations
    )
    ...
    for i in range(n_iterations):
        ...
        print(
            f"   Iteration {i+1}: {confidence:.3f} ({(time.perf_counter() - start)*1000:.1f} ms)"
        )
    ...
    for i in range(n_iterations):
        ...
        print(
            f"   Iteration {i+1}: {len(gaps)} gaps ({(time.perf_counter() - start)*1000:.1f} ms)"
        )
    ...
    for i in range(n_iterations):
        ...
        print(
            f"   Iteration {i+1}: {val_score:.3f} ({(time.perf_counter() - start)*1000:.1f} ms)"
        )
    ...
    scored_links = await strategy.select_links_for_expansion(
        mock_links, gaps, state.kb_embeddings
    )
    metrics.record_operation("select_links", time.perf_counter() - start)
    print(
        f"   Scored {len(scored_links)} links in {(time.perf_counter() - start)*1000:.1f} ms"
    )
...
async def main():
    ...
    calc_conf_avg = sum(
        metrics_before.operation_times.get("calculate_confidence", [])
    ) / len(metrics_before.operation_times.get("calculate_confidence", [1]))
    find_gaps_avg = sum(
        metrics_before.operation_times.get("find_coverage_gaps", [])
    ) / len(metrics_before.operation_times.get("find_coverage_gaps", [1]))
    validate_avg = sum(
        metrics_before.operation_times.get("validate_coverage", [])
    ) / len(metrics_before.operation_times.get("validate_coverage", [1]))
