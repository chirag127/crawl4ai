...
def demo_part1_pattern_matching():
    ...
    json_or_api_config = CrawlerRunConfig(
        url_matcher=["*.json", lambda url: "api" in url],  # String pattern  # Function
    ...
    complex_config = CrawlerRunConfig(
        url_matcher=[
            ...
            lambda url: any(
                pattern in url for pattern in ["/blog/", "/article/"]
            ),  # Function: Blog OR article
...
async def demo_part2_practical_crawling():
    ...
    configs = [
        ...
        CrawlerRunConfig(
            url_matcher="*.pdf", scraping_strategy=PDFContentScrapingStrategy()
        ),
        ...
        CrawlerRunConfig(
            url_matcher=[
                ...
                lambda url: "api" in url
                or "httpbin.org" in url,  # Function for API endpoints
            ],
            match_mode=MatchMode.OR,
        ),
        ...
        CrawlerRunConfig(
            url_matcher=[
                ...
                lambda url: any(
                    doc in url for doc in ["docs", "documentation", "reference"]
                ),  # Has docs
        ...
        for result in results:
            if result.success:
                ...
                if result.url.endswith(".pdf"):
                    ...
                    any(pattern in result.url for pattern in ["blog", "python.org"])
                    and "docs" not in result.url
                ):
                ...
                if (
                    ...
                    print(
                        f"  Fit markdown size: {len(result.markdown.fit_markdown)} chars"
                    )
                    reduction = (
                        1 - len(result.markdown.fit_markdown) / len(result.markdown)
                    ) * 100
