...
class BrowserProfiler:
    ...
    async def _listen_unix(
        self, user_done_event: asyncio.Event, check_browser_process, tag: str
    ):
        ...
        try:
            ...
            while True:
                try:
                    ...
                    self.logger.warning(
                        f"Error in Unix keyboard listener: {e}", tag=tag
                    )
                    await asyncio.sleep(0.1)
                    continue
            ...
            if old_settings is not None:
                try:
                    ...
                    self.logger.error(
                        f"Failed to restore terminal settings: {e}", tag=tag
                    )

    async def _listen_fallback(
        self, user_done_event: asyncio.Event, check_browser_process, tag: str
    ):
        """Fallback keyboard listener using simple input() method."""
        self.logger.info(
            "Using fallback input mode. Type 'q' and press Enter to quit.", tag=tag
        )
            ...
            try:
                while not user_done_event.is_set():
                    try:
                        ...
                        user_input = (
                            input("Press 'q' + Enter to quit: ").strip().lower()
                        )
    ...
    async def create_profile(
        ...
        profile_name = "".join(
            c if c.isalnum() or c in "-_" else "_" for c in profile_name
        )
        ...
        self.logger.info(
            "Creating browser profile: {profile_name}",
            tag="PROFILE",
            params={"profile_name": profile_name},
            colors={"profile_name": LogColor.GREEN},
        )
        self.logger.info(
            "Profile directory: {profile_path}",
            tag="PROFILE",
            params={"profile_path": profile_path},
            colors={"profile_path": LogColor.YELLOW},
        )
        ...
        self.logger.info(
            "{segment}, configure settings, etc. as needed.",
            tag="PROFILE",
            params={"segment": "2. Log in to websites"},
            colors={"segment": LogColor.CYAN},
        )
        self.logger.info(
            "3. When you're done, {segment} to close the browser.",
            tag="PROFILE",
            params={"segment": "press 'q' in this terminal"},
            colors={"segment": LogColor.YELLOW},
        )
        self.logger.info(
        ...
        async def listen_for_quit_command():
            ...
            try:
                if self._is_windows():
                    await self._listen_windows(
                        user_done_event, check_browser_process, "PROFILE"
                    )
                else:
                    await self._listen_unix(
                        user_done_event, check_browser_process, "PROFILE"
                    )
            except Exception as e:
                self.logger.warning(
                    f"Platform-specific keyboard listener failed: {e}", tag="PROFILE"
                )
                self.logger.info("Falling back to simple input mode...", tag="PROFILE")
                await self._listen_fallback(
                    user_done_event, check_browser_process, "PROFILE"
                )
        ...
        try:
            ...
            self.logger.info(
                "Browser launched. Waiting for you to finish...", tag="PROFILE"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"[PROFILE].i  storage_state saved → {state_file}", tag="PROFILE"
                )
            except Exception as e:
                self.logger.warning(
                    f"[PROFILE].w  failed to save storage_state: {e}", tag="PROFILE"
                )
            ...
            self.logger.success(
                f"Browser closed. Profile saved at: {profile_path}", tag="PROFILE"
            )
        ...
        if shrink_level != ShrinkLevel.NONE and profile_path:
            self.logger.info(
                f"Shrinking profile with level: {shrink_level.value}", tag="PROFILE"
            )
    ...
    def list_profiles(self) -> List[Dict[str, Any]]:
        ...
        for name in os.listdir(self.profiles_dir):
            ...
            ) or os.path.exists(os.path.join(profile_path, "Default", "Preferences")):
                is_valid = "chromium"
    ...
    def get_profile_path(self, profile_name: str) -> Optional[str]:
        ...
        is_profile = (
            os.path.exists(os.path.join(profile_path, "Preferences"))
            or os.path.exists(os.path.join(profile_path, "Default", "Preferences"))
            or os.path.exists(os.path.join(profile_path, "prefs.js"))
        )
    ...
    async def interactive_manager(self, crawl_callback=None):
        ...
        while True:
            ...
            self.logger.info(
                "1. Create a new profile", tag="MENU", base_color=LogColor.GREEN
            )
            self.logger.info(
                "2. List available profiles", tag="MENU", base_color=LogColor.YELLOW
            )
            ...
            if choice == "1":
                # Create new profile
                self.console.print(
                    "[green]Enter a name for the new profile (or press Enter for auto-generated name): [/green]",
                    end="",
                )
                name = input()
                ...
                for i, profile in enumerate(profiles):
                    ...
                    self.logger.info(
                        f"    Browser type: {profile['type']}", tag="PROFILES"
                    )
                ...
                self.console.print(
                    "[red]Enter the number of the profile to delete (or 'c' to cancel): [/red]",
                    end="",
                )
                profile_idx = input()
                ...
                try:
                    ...
                    if 0 <= idx < len(profiles):
                        ...
                        self.logger.info(
                            f"Deleting profile: [yellow]{profile_name}[/yellow]",
                            tag="PROFILES",
                        )

                        # Confirm deletion
                        self.console.print(
                            "[red]Are you sure you want to delete this profile? (y/n): [/red]",
                            end="",
                        )
                        confirm = input()
                        if confirm.lower() == "y":
                ...
                if not profiles:
                    self.logger.warning(
                        "No profiles found. Create one first.", tag="PROFILES"
                    )
                    continue
                ...
                # Get profile to use
                self.console.print(
                    "[cyan]Enter the number of the profile to use (or 'c' to cancel): [/cyan]",
                    end="",
                )
                profile_idx = input()
                if profile_idx.lower() == "c":
                ...
                try:
                    ...
                    if 0 <= idx < len(profiles):
                        ...
                        self.console.print(
                            "[cyan]Enter the URL to crawl: [/cyan]", end=""
                        )
                        url = input()
                        if url:
    ...
    async def launch_standalone_browser(
        ...
        self.logger.info(
            ...
            colors={"browser_type": LogColor.CYAN},
        )
        self.logger.info(
            "Profile path: {profile_path}",
            tag="CDP",
        ...
        async def listen_for_quit_command():
            ...
            async def check_browser_process():
                ...
                if (
                    ...
                    self.logger.info(
                        "Browser already closed. Ending input listener.", tag="CDP"
                    )
                    user_done_event.set()
                    return True
                return False
            ...
            try:
                if self._is_windows():
                    await self._listen_windows(
                        user_done_event, check_browser_process, "CDP"
                    )
                else:
                    await self._listen_unix(
                        user_done_event, check_browser_process, "CDP"
                ...
                self.logger.warning(
                    f"Platform-specific keyboard listener failed: {e}", tag="CDP"
                )
        ...
        try:
            ...
            self.logger.info(
                "Browser launched successfully. Retrieving CDP information...",
                tag="CDP",
            )
            ...
            if cdp_url:
                ...
                if config_json:
                    ...
                    self.logger.info(
                        f"Browser: {config_json.get('Browser', 'Unknown')}",
                        tag="CDP",
                        colors={"Browser": LogColor.CYAN},
                    )
                    self.logger.info(
                        f"Protocol Version: {config_json.get('Protocol-Version', 'Unknown')}",
                        tag="CDP",
                        colors={"Protocol-Version": LogColor.CYAN},
                    )
                    if "webSocketDebuggerUrl" in config_json:
                        self.logger.info(
                            "WebSocket URL: {webSocketDebuggerUrl}",
                            tag="CDP",
                            params={
                                "webSocketDebuggerUrl": config_json[
                                    "webSocketDebuggerUrl"
                                ]
                            },
                            colors={"webSocketDebuggerUrl": LogColor.GREEN},
                        )
                else:
                    self.logger.warning(
                        "Could not retrieve CDP configuration JSON", tag="CDP"
                    )
            else:
                self.logger.error(
                    f"Failed to get CDP URL on port {debugging_port}", tag="CDP"
                )
                await managed_browser.cleanup()
                return None
            ...
            self.logger.error(
                f"Error launching standalone browser: {str(e)}", tag="CDP"
            )
            await managed_browser.cleanup()
            return None
        finally:
    ...
    async def launch_builtin_browser(
        ...
        try:
            ...
            self.logger.success(
                f"Builtin browser launched at CDP URL: {cdp_url}", tag="BUILTIN"
            )
            return cdp_url

        except Exception as e:
            self.logger.error(
                f"Error launching builtin browser: {str(e)}", tag="BUILTIN"
            )
            if managed_browser:
                await managed_browser.cleanup()
            return None
    ...
    def get_builtin_browser_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                f"Error reading builtin browser config: {str(e)}", tag="BUILTIN"
            )
            return None
    ...
    async def get_builtin_browser_status(self) -> Dict[str, Any]:
        ...
        if not browser_info:
            return {"running": False, "cdp_url": None, "info": None}
...
if __name__ == "__main__":
    ...
    profile_path = asyncio.run(
        profiler.create_profile(str(home_dir / ".crawl4ai/profiles/test-profile"))
    )
