...
def test_content_filters() -> Dict[str, Dict[str, int]]:
    ...
    bm25_filter = BM25ContentFilter(
        bm25_threshold=1.0, user_query="test article content important"
    )
    ...
    for test_name, html in TEST_HTML_SAMPLES.items():
        for gen_name, generator in generators.items():
            ...
            result = generator.generate_markdown(
                html, base_url="http://example.com", citations=True
            )
            ...
            results.append(
                {
                    ...
                    "fit_length": (
                        len(result.fit_markdown) if result.fit_markdown else 0
                    ),
...
def main():
    ...
    for test_name, metrics in filter_results.items():
        ...
        print(
            f"Pruned length: {metrics['pruned_length']} ({metrics['pruning_time']:.3f}s)"
        )
