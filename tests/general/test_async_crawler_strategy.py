...
def basic_crawler_config():
    return CrawlerRunConfig(
        word_count_threshold=100, wait_until="domcontentloaded", page_timeout=30000
    )
...
async def crawler_strategy(basic_browser_config, logger):
    strategy = AsyncPlaywrightCrawlerStrategy(
        browser_config=basic_browser_config, logger=logger
    )
...
async def test_browser_config_initialization():
    config = BrowserConfig(browser_type="chromium", user_agent_mode="random")
...
async def test_persistent_browser_config():
    config = BrowserConfig(use_persistent_context=True, user_data_dir="/tmp/test_dir")
...
async def test_basic_page_load(crawler_strategy):
    response = await crawler_strategy.crawl("https://example.com", CrawlerRunConfig())
...
async def test_screenshot_capture(crawler_strategy):
    ...
    response = await crawler_strategy.crawl("https://example.com", config)
...
async def test_handle_js_execution(crawler_strategy):
    config = CrawlerRunConfig(js_code="document.body.style.backgroundColor = 'red';")
    response = await crawler_strategy.crawl("https://example.com", config)
    assert response.status_code == 200
...
async def test_session_management(crawler_strategy):
    ...
    response1 = await crawler_strategy.crawl("https://example.com", config)
    response2 = await crawler_strategy.crawl("https://example.com", config)
...
async def test_process_iframes(crawler_strategy):
    config = CrawlerRunConfig(process_iframes=True, wait_for_images=True)
    response = await crawler_strategy.crawl("https://example.com", config)
    assert response.status_code == 200
...
async def test_stealth_mode(crawler_strategy):
    config = CrawlerRunConfig(simulate_user=True, override_navigator=True)
    response = await crawler_strategy.crawl("https://bot.sannysoft.com", config)
    assert response.status_code == 200
...
async def test_remove_overlay_elements(crawler_strategy):
    ...
    response = await crawler_strategy.crawl(
        "https://www2.hm.com/en_us/index.html", config
    )
    assert response.status_code == 200
