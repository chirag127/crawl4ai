...
from crawl4ai import (AsyncWebCrawler, BrowserConfig, CrawlerRunConfig,
                      CrawlResult, PlaywrightAdapter, UndetectedAdapter)
...
async def compare_adapters(url: str, site_name: str):
    ...
    print(
        f"Regular Adapter: {'✅ Passed' if regular_success else '❌ Blocked/Detected'}"
    )
    print(
        f"Undetected Adapter: {'✅ Passed' if undetected_success else '❌ Blocked/Detected'}"
    )
    print(f"{'='*60}")
...
async def main():
    ...
    for site_name, url in TEST_SITES.items():
        ...
        results[site_name] = {"regular": regular, "undetected": undetected}
    ...
    print("Success Rates:")
    print(
        f"Regular Adapter: {regular_success}/{total} ({regular_success/total*100:.1f}%)"
    )
    print(
        f"Undetected Adapter: {undetected_success}/{total} ({undetected_success/total*100:.1f}%)"
    )
    print(f"{'='*60}")
