...
async def test_max_scroll_steps():
    ...
    async with AsyncWebCrawler(verbose=True) as crawler:
        ...
        print(
            f"Config: scan_full_page={config1.scan_full_page}, max_scroll_steps={config1.max_scroll_steps}"
        )

        try:
            result1 = await crawler.arun(
                url="https://example.com", config=config1  # Simple page for testing
            )
        ...
        print(
            f"Config: scan_full_page={config2.scan_full_page}, max_scroll_steps={config2.max_scroll_steps}"
        )

        try:
            result2 = await crawler.arun(
                url="https://techcrunch.com/", config=config2  # Another test page
            )
        ...
        config3 = CrawlerRunConfig(
            scan_full_page=True, max_scroll_steps=5, scroll_delay=0.2
        )
        ...
        config4 = CrawlerRunConfig.from_kwargs(
            {"scan_full_page": True, "max_scroll_steps": 7, "scroll_delay": 0.3}
        )
        ...
        try:
            result5 = await crawler.arun(url="https://techcrunch.com/", config=config5)
            print(
                f"✅ Test 4 Success: No scrolling performed, crawled {len(result5.markdown)} characters"
            )
        except Exception as e:
