...
async def start_test_server():
    ...
    async def image(request):
        ...
        return web.Response(
            body=bytes.fromhex(
                "89504E470D0A1A0A0000000D49484452000000010000000108060000001F15C4890000000D4944415478DA63FAFFFF3F030079DB00018D959DE70000000049454E44AE426082"
            ),
...
async def test_network_console_capture():
    ...
    try:
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            assert (
                result.network_requests is None
            ), "Network requests should be None when capture is disabled"
            assert (
                result.console_messages is None
            ), "Console messages should be None when capture is disabled"
            print(
                "✓ Default config correctly returns None for network_requests and console_messages"
            )
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            request_count = len(
                [r for r in result.network_requests if r.get("event_type") == "request"]
            )
            response_count = len(
                [
                    r
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            message_types = set(
                msg.get("type") for msg in result.console_messages if "type" in msg
            )
            ...
            for msg in result.console_messages:
                print(
                    f"    * Type: {msg.get('type', 'N/A')}, Text: {msg.get('text', 'N/A')}"
                )

            # Look for specific messages
            messages = [
                msg.get("text") for msg in result.console_messages if "text" in msg
            ]
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            print(
                f"✓ Successfully captured both {len(result.network_requests)} network requests and {len(result.console_messages)} console messages"
            )
