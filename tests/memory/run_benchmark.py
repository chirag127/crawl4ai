...
TEST_CONFIGS = {
    "quick": {
        ...
        "max_sessions": 4,
        "chunk_size": 10,
        "description": "Quick test (50 URLs, 4 sessions)",
    },
    "small": {
        "urls": 100,
        "max_sessions": 8,
        "chunk_size": 20,
        "description": "Small test (100 URLs, 8 sessions)",
    },
    "medium": {
        "urls": 500,
        "max_sessions": 16,
        "chunk_size": 50,
        "description": "Medium test (500 URLs, 16 sessions)",
    },
    "large": {
        "urls": 1000,
        "max_sessions": 32,
        "chunk_size": 100,
        "description": "Large test (1000 URLs, 32 sessions)",
    },
    "extreme": {
        "urls": 2000,
        "max_sessions": 64,
        "chunk_size": 200,
        "description": "Extreme test (2000 URLs, 64 sessions)",
    },
}
...
def run_benchmark(config_name, custom_args=None, compare=True, clean=False):
    ...
    if clean:
        ...
        console.print(
            "[yellow]Applying --clean: Cleaning reports and site before test.[/yellow]"
        )
    ...
    try:
        ...
        console.print(
            "[bold red]Error: Script 'test_stress_sdk.py' not found. Make sure it's in the correct directory.[/bold red]"
        )
        return False
    ...
    if proc.returncode != 0:
        console.print(
            f"[bold red]Stress test failed with exit code {proc.returncode}[/bold red]"
        )
        return False

    duration = time.time() - start
    console.print(
        f"[bold green]Stress test completed in {duration:.1f} seconds[/bold green]"
    )
    ...
    if compare:
        ...
        try:
            report_proc = subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
            )  # Use check=False to handle potential errors
            ...
            if report_proc.stderr:
                console.print(
                    "[yellow]Report generator stderr:[/yellow]\n" + report_proc.stderr
                )

            if report_proc.returncode != 0:
                console.print(
                    f"[bold yellow]Benchmark report generation script '{report_script}' failed with exit code {report_proc.returncode}[/bold yellow]"
                )
                # Don't return False here, test itself succeeded
            else:
                console.print(
                    f"[bold green]Benchmark report script '{report_script}' completed.[/bold green]"
                )
            ...
            if os.path.isdir(report_dir):
                report_files = glob.glob(
                    os.path.join(report_dir, "comparison_report_*.html")
                )
                if report_files:
                    try:
                        ...
                        report_url = pathlib.Path(
                            report_path
                        ).as_uri()  # Better way to create file URI
                        console.print(
                            f"[bold cyan]Click to open report: [link={report_url}]{report_url}[/link][/bold cyan]"
                        )
                    except Exception as e:
                        console.print(
                            f"[yellow]Could not determine latest report: {e}[/yellow]"
                        )
                ...
                if chart_files:
                    try:
                        ...
                        console.print(
                            f"[cyan]Memory chart: [link={chart_url}]{chart_url}[/link][/cyan]"
                        )
                    except Exception as e:
                        console.print(
                            f"[yellow]Could not determine latest chart: {e}[/yellow]"
                        )
            else:
                console.print(
                    f"[yellow]Benchmark report directory '{report_dir}' not found. Cannot link reports.[/yellow]"
                )

        except FileNotFoundError:
            console.print(
                f"[bold red]Error: Report script '{report_script}' not found.[/bold red]"
            )
        except Exception as e:
            console.print(
                f"[bold red]Error running report generation subprocess: {e}[/bold red]"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a Crawl4AI SDK benchmark test and generate a report"
    )

    # --- Arguments ---
    parser.add_argument(
        "config",
        choices=list(TEST_CONFIGS) + ["custom"],
    ...
    parser.add_argument(
        "--max-sessions", type=int, help="Max concurrent sessions (replaces --workers)"
    )
    parser.add_argument(
        "--chunk-size", type=int, help="URLs per batch (for non-stream logging)"
    )
    ...
    parser.add_argument(
        "--stream",
        action="store_true",
        help="Enable streaming results (disables batch logging)",
    )
    parser.add_argument(
        "--use-rate-limiter", action="store_true", help="Enable basic rate limiter"
    )
    parser.add_argument(
        "--no-report", action="store_true", help="Skip generating comparison report"
    )
    parser.add_argument(
        "--clean", action="store_true", help="Clean up reports and site before running"
    )
    parser.add_argument(
        "--keep-server-alive",
        action="store_true",
        help="Keep HTTP server running after test",
    )
    parser.add_argument(
        "--use-existing-site",
        action="store_true",
        help="Use existing site on specified port",
    )
    parser.add_argument(
        "--skip-generation",
        action="store_true",
        help="Use existing site files without regenerating",
    )
    parser.add_argument(
        "--keep-site", action="store_true", help="Keep generated site files after test"
    )
    ...
    if args.urls is not None:
        custom_args["urls"] = args.urls
    if args.max_sessions is not None:
        custom_args["max_sessions"] = args.max_sessions
    if args.chunk_size is not None:
        custom_args["chunk_size"] = args.chunk_size
    if args.port is not None:
        custom_args["port"] = args.port
    if args.monitor_mode is not None:
        custom_args["monitor_mode"] = args.monitor_mode
    if args.stream:
        custom_args["stream"] = True
    if args.use_rate_limiter:
        custom_args["use_rate_limiter"] = True
    if args.keep_server_alive:
        custom_args["keep_server_alive"] = True
    if args.use_existing_site:
        custom_args["use_existing_site"] = True
    if args.skip_generation:
        custom_args["skip_generation"] = True
    if args.keep_site:
        custom_args["keep_site"] = True
    ...
    if args.config == "custom":
        ...
        if missing:
            console.print(
                f"[bold red]Error: 'custom' config requires: {', '.join(missing)}[/bold red]"
            )
            return 1
