...
from crawl4ai.antibot_detector import (_looks_like_data,
                                       is_blocked)
...
check(
    "normal HTML page",
    _looks_like_data("<html><body><p>Hello</p></body></html>"),
    False,
)
check(
    "<!DOCTYPE html>",
    _looks_like_data("<!DOCTYPE html><html><body></body></html>"),
    False,
)
...
check(
    "browser-wrapped JSON object",
    _looks_like_data(
        '<html><head></head><body><pre style="word-wrap: break-word; white-space: pre-wrap;">{"origin": "1.2.3.4"}</pre></body></html>'
    ),
    True,
)
...
check(
    ...
    _looks_like_data(
        '<html><head></head><body><pre>[{"id": 1}, {"id": 2}]</pre></body></html>'
    ),
    True,
)

check(
    "browser-wrapped JSON (uppercase HTML)",
    _looks_like_data(
        '<HTML><HEAD></HEAD><BODY><PRE>{"key": "val"}</PRE></BODY></HTML>'
    ),
    True,
)

check(
    "browser-wrapped JSON (DOCTYPE)",
    _looks_like_data(
        '<!DOCTYPE html><html><head></head><body><pre>{"x": 1}</pre></body></html>'
    ),
    True,
)

check(
    "browser-wrapped JSON with whitespace before pre",
    _looks_like_data(
...
check(
    "HTML page with code block (not JSON in pre)",
    _looks_like_data(
        '<html><body><h1>Tutorial</h1><pre>def hello():\n    print("hi")</pre></body></html>'
    ),
    False,
)

check(
    "HTML with <pre> but text content, not JSON",
    _looks_like_data(
        "<html><body><pre>This is just preformatted text, not JSON.</pre></body></html>"
    ),
    False,
)
...
blocked, reason = is_blocked(
    200,
    '<html><head></head><body><script>window.location="/challenge"</script></body></html>',
)
...
blocked, reason = is_blocked(
    403,
    "<html><body><pre>Reference #18.2d351ab8.1557333295.a4e16ab</pre></body></html>",
)
check("Akamai ref in <pre> (403)", blocked, True, reason)

blocked, reason = is_blocked(
    200, '<html><body><pre>window._pxAppId = "PX123";</pre></body></html>'
)
...
check(
    "/docs/* matches /docs/sub/page", f.apply("https://example.com/docs/sub/page"), True
)
...
check(
    "full URL prefix matches subpath",
    f4.apply("https://example.com/blog/2024/post-1"),
    True,
)
f5 = URLPatternFilter(patterns=["https://example.com/blog/*"])
check(
    "full URL prefix no match different domain",
    f5.apply("https://other.com/blog/post-1"),
    False,
)
f6 = URLPatternFilter(patterns=["https://example.com/blog/*"])
check(
    "full URL prefix no match blogxx",
    f6.apply("https://example.com/blogxx/post-1"),
    False,
)
...
check(
    "/api/* matches /api/v1?key=123",
    f7.apply("https://example.com/api/v1?key=123"),
    True,
)
...
check(
    "reverse: /private/* excludes /private/page",
    f11.apply("https://example.com/private/page"),
    False,
)
f12 = URLPatternFilter(patterns=["/private/*"], reverse=True)
check(
    "reverse: /private/* allows /public/page",
    f12.apply("https://example.com/public/page"),
    True,
)
...
check(
    "prefix with auth in URL",
    f14.apply("https://user:pass@example.com/docs/page"),
    True,
)
...
check(
    "multi-prefix: /other/ no match", f18.apply("https://example.com/other/page"), False
)
...
check(
    "glob */docs/*/guide matches", f19.apply("https://example.com/docs/v2/guide"), True
)
f20 = URLPatternFilter(patterns=["*/docs/*/guide"])
check(
    "glob */docs/*/guide no match",
    f20.apply("https://example.com/docs/v2/tutorial"),
    False,
)
...
check(
    "domain http://*.example.com/* matches sub.example.com",
    f21.apply("http://sub.example.com/page"),
    True,
)
f22 = URLPatternFilter(patterns=["http://*.example.com/*"])
check(
    "domain http://*.example.com/* no match other.com",
    f22.apply("http://other.com/page"),
    False,
)
...
check(
    "PDFContentScrapingStrategy in ALLOWED_DESERIALIZE_TYPES",
    "PDFContentScrapingStrategy" in ALLOWED_DESERIALIZE_TYPES,
    True,
)
...
try:
    ...
    check(
        "serialization type field", serialized.get("type"), "PDFContentScrapingStrategy"
    )
    ...
    check(
        "deserialization creates valid processor",
        hasattr(deserialized, "pdf_processor"),
        True,
    )
...
try:
    ...
    deserialized = from_serializable_dict(serialized)
    check(
        "CrawlerRunConfig roundtrip with PDF strategy",
        type(deserialized.scraping_strategy).__name__,
        "PDFContentScrapingStrategy",
    )
    check(
        "PDF strategy has processor after roundtrip",
        hasattr(deserialized.scraping_strategy, "pdf_processor"),
        True,
    )
...
try:
    ...
    deserialized = from_serializable_dict(serialized)
    check(
        "CrawlerRunConfig basic roundtrip type",
        type(deserialized).__name__,
        "CrawlerRunConfig",
    )
    check(
        "CrawlerRunConfig cache_mode preserved",
        deserialized.cache_mode,
        CacheMode.BYPASS,
    )
...
blocked, reason = is_blocked(
    200,
    "<html><body><pre>{not valid json at all, this is just text</pre></body></html>",
)
...
check(
    "/docs/* matches /docs exactly (prefix == path)",
    f_edge4.apply("https://example.com/docs"),
    True,
)
