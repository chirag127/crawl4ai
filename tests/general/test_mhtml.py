...
EXPECTED_CONTENT_JS = (
    "Quotes to Scrape"  # Title of the page, which should be present in MHTML
)
...
async def test_mhtml_capture_when_enabled():
    ...
    try:
        ...
        ), f"Crawling {TEST_URL_SIMPLE} should succeed. Error: {result.error_message}"

        # 1. Check if the mhtml attribute exists (will fail if CrawlResult not updated)
        assert hasattr(
            result, "mhtml"
        ), "CrawlResult object must have an 'mhtml' attribute"
        ...
        assert (
            EXPECTED_CONTENT_SIMPLE in result.mhtml
        ...
        assert (
            EXPECTED_CONTENT_SIMPLE in result.html
...
async def test_mhtml_capture_when_disabled_explicitly():
    ...
    try:
        ...
        assert (
            result.mhtml is None
        ), "MHTML content should be None when explicitly disabled"
...
async def test_mhtml_capture_when_disabled_by_default():
    ...
    try:
        ...
        assert (
            result.mhtml is None
        ), "MHTML content should be None when using default config (assuming default=False)"
...
async def test_mhtml_capture_on_js_page_when_enabled():
    ...
    try:
        ...
        assert (
            result.success is True
        ), f"Crawling {TEST_URL_JS} should succeed. Error: {result.error_message}"
        assert hasattr(
            result, "mhtml"
        ), "CrawlResult object must have an 'mhtml' attribute"
        assert (
            result.mhtml is not None
        ), "MHTML content should be captured on JS page when enabled"
        assert isinstance(result.mhtml, str), "MHTML content should be a string"
