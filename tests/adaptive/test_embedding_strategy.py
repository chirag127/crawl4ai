...
async def test_basic_embedding_crawl():
    ...
    if not llm_config["api_token"]:
        console.print(
            "[red]Warning: OPENAI_API_KEY not set. Using mock data for demo.[/red]"
        )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        console.print(
            f"Is Sufficient: {'Yes (Validated)' if prog_crawler.is_sufficient else 'No'}"
        )
...
async def test_embedding_vs_statistical(use_openai=False):
    ...
    config_stat = AdaptiveConfig(
        strategy="statistical", confidence_threshold=0.7, max_pages=10
    )
    ...
    console.print(
        f"Statistical: {stat_pages} pages in {stat_time:.2f}s, confidence: {stat_confidence:.2%}, sufficient: {stat_crawler.is_sufficient}"
    )
    console.print(
        f"Embedding:   {emb_pages} pages in {emb_time:.2f}s, confidence: {emb_confidence:.2%}, sufficient: {emb_crawler.is_sufficient}"
    )
    ...
    if hasattr(state_emb, "metrics") and "validation_confidence" in state_emb.metrics:
        console.print(
            f"Embedding validation score: {state_emb.metrics['validation_confidence']:.2%}"
        )
...
async def test_custom_embedding_provider():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        state = await prog_crawler.digest(
            start_url="https://httpbin.org", query="api endpoints json response"
        )
...
async def test_knowledge_export_import():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        console.print(
            f"[green]Exported {len(state1.knowledge_base)} documents with embeddings[/green]"
        )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        state2 = await crawler2.digest(
            start_url="https://httpbin.org", query="authentication oauth tokens"
        )

        console.print(
            f"[green]Total knowledge base: {len(state2.knowledge_base)} documents[/green]"
        )
    ...
    for test_case in test_cases:
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            state = await emb_crawler.digest(
                start_url=test_case["url"], query=test_case["query"]
            )
            ...
            for i in range(0, total_pages, 3):
                ...
                estimated_confidence = state.metrics.get("confidence", 0) * (
                    pages_so_far / total_pages
                )

                console.print(
                    f"Batch {batch_num}: {batch_pages} pages → Confidence: {estimated_confidence:.1%} {'✅' if estimated_confidence >= 0.8 else '❌'}"
                )

            final_confidence = emb_crawler.confidence
            console.print(
                f"[green]Final: {total_pages} pages → Confidence: {final_confidence:.1%} {'✅ (Sufficient!)' if emb_crawler.is_sufficient else '❌'}[/green]"
            )

            # Show learning metrics for embedding
            if "avg_min_distance" in state.metrics:
                console.print(
                    f"[dim]Avg gap distance: {state.metrics['avg_min_distance']:.3f}[/dim]"
                )
            if "validation_confidence" in state.metrics:
                console.print(
                    f"[dim]Validation score: {state.metrics['validation_confidence']:.1%}[/dim]"
                )
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            console.print(
                f"[green]Final: {total_pages} pages → Confidence: {final_confidence:.1%} {'✅ (Sufficient!)' if stat_crawler.is_sufficient else '❌'}[/green]"
            )
...
async def test_irrelevant_query_behavior():
    ...
    console.print(
        "Testing embedding strategy with a query that has no semantic relevance to the content"
    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        console.print(
            f"Final confidence: {final_confidence:.1%} {'✅' if emb_crawler.is_sufficient else '❌'}"
        )
        ...
        if "avg_min_distance" in state.metrics:
            ...
            console.print(
                f"  Average minimum distance: {state.metrics['avg_min_distance']:.3f}"
            )
            console.print(
                f"  Close neighbors (<0.3): {state.metrics.get('avg_close_neighbors', 0):.1f}"
            )
            console.print(
                f"  Very close neighbors (<0.2): {state.metrics.get('avg_very_close_neighbors', 0):.1f}"
            )
        ...
        if "validation_confidence" in state.metrics:
            ...
            console.print(
                f"  Validation score: {state.metrics['validation_confidence']:.1%}"
            )
        ...
        if "stopped_reason" in state.metrics:
            ...
            if state.metrics.get("is_irrelevant", False):
                console.print(
                    "[red]→ Query and content are completely unrelated![/red]"
                )
        ...
        if final_confidence < 0.2:
            console.print(
                "[red]✗ As expected: Query is completely irrelevant to content[/red]"
            )
            console.print(
                "[green]✓ The embedding strategy correctly identified no semantic match[/green]"
            )
        else:
            console.print(
                f"[yellow]⚠ Unexpected: Got {final_confidence:.1%} confidence for irrelevant query[/yellow]"
            )
            console.print(
                "[yellow]  This may indicate the query variations are too broad[/yellow]"
            )
...
async def test_high_dimensional_handling():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        try:
            state = await prog_crawler.digest(
                start_url="https://httpbin.org", query="api endpoints json"
            )

            console.print(
                f"[green]✓ Successfully handled {len(state.expanded_queries)} queries in 384D space[/green]"
            )
            ...
            if isinstance(state.coverage_shape, dict):
                ...
                console.print(
                    f"  - Center shape: {state.coverage_shape['center'].shape if 'center' in state.coverage_shape else 'N/A'}"
                )
                console.print(
                    f"  - Radius: {state.coverage_shape.get('radius', 'N/A'):.3f}"
                )

        except Exception as e:
            console.print(f"[red]Error: {e}[/red]")
            console.print(
                "[yellow]This demonstrates why alpha shapes don't work in high dimensions[/yellow]"
            )
...
async def main():
    ...
    console.print(
        "[bold magenta]Embedding-based Adaptive Crawler Test Suite[/bold magenta]"
    )
    console.print("=" * 60)
    ...
    try:
        ...
        try:
            ...
            console.print(
                "[yellow]Warning: sentence-transformers not available[/yellow]"
            )
            console.print(
                "[yellow]Tests will use OpenAI embeddings if available or mock data[/yellow]"
            )
        ...
        if has_numpy:
            ...
            if not has_sentence_transformers and not os.getenv("OPENAI_API_KEY"):
                ...
                console.print(
                    "[yellow]Please set OPENAI_API_KEY or fix sentence-transformers installation[/yellow]"
                )
                return
