...
async def extract_pipeline_v2(
    ...
    async with AsyncWebCrawler(verbose=False) as crawler:
        ...
        if force_llm:
            ...
            vprint(
                f"Classification: {classification_data['classification']} (confidence: {classification_data['confidence']})"
            )
        ...
        if not target_json_example and "suggested_json_example" in classification_data:
            target_json_example = json.dumps(
                classification_data["suggested_json_example"]
            )
...
async def main():
    ...
    try:
        ...
        target_example = json.dumps(
            {"title": "Issue title here", "number": "#123", "author": "username"}
        )
