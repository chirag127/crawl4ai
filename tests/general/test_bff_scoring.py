...
async def test_best_first_strategy():
    ...
    browser_config = BrowserConfig(
        headless=True, verbose=False  # Run in background  # Reduce output noise
    )

    crawler_config = CrawlerRunConfig(deep_crawl_strategy=strategy, verbose=False)
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        results = await crawler.arun(
            url="https://docs.python.org/3/", config=crawler_config
        )
    ...
    for i, item in enumerate(crawled_urls, 1):
        ...
        if item["score"] > 0.5:
            print(
                f"{i:2}. [{status}] Score: {item['score']:.2f} | Depth: {item['depth']} | {item['url']}"
            )
    ...
    print(
        f"  - Average score: {sum(item['score'] for item in crawled_urls) / len(crawled_urls):.2f}"
    )
    print(f"  - Max score: {max(item['score'] for item in crawled_urls):.2f}")
    print(
        f"  - Pages with keywords: {sum(1 for item in crawled_urls if item['score'] > 0.3)}"
    )
