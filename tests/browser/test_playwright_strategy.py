...
async def test_playwright_text_mode():
    ...
    browser_config = BrowserConfig(
        headless=True, text_mode=True  # Enable text-only mode
    )
    ...
    try:
        ...
        async with page.expect_request(
            "**/*.{png,jpg,jpeg,gif,webp,svg}", timeout=1000
        ) as request_info:
            try:
...
async def test_playwright_context_reuse():
    ...
    try:
        ...
        logger.info(
            f"Different contexts for different configs: {is_different_context}",
            tag="TEST",
        )
...
async def test_playwright_session_management():
    ...
    try:
        ...
        crawler_config1 = CrawlerRunConfig(
            session_id=session1_id, url="https://example.com"
        )
        page1, context1 = await manager.get_page(crawler_config1)
        await page1.goto("https://example.com")
        await page1.evaluate(
            "localStorage.setItem('playwright_session1_data', 'test_value1')"
        )
        logger.info(f"Set up session 1 with ID: {session1_id}", tag="TEST")

        # Set up second session
        crawler_config2 = CrawlerRunConfig(
            session_id=session2_id, url="https://example.org"
        )
        page2, context2 = await manager.get_page(crawler_config2)
        await page2.goto("https://example.org")
        await page2.evaluate(
            "localStorage.setItem('playwright_session2_data', 'test_value2')"
        )
        ...
        data1 = await page1_again.evaluate(
            "localStorage.getItem('playwright_session1_data')"
        )
        logger.info(
            f"Session 1 reuse successful: {is_same_page}, data: {data1}", tag="TEST"
        )
        ...
        logger.info(
            f"Session 2 still functional after killing session 1, data: {data2}",
            tag="TEST",
        )
