...
async def crawl_with_regular_browser(url: str):
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        result = await crawler.arun(
            url=url, config=CrawlerRunConfig(delay_before_return_html=2.0)
        )
        ...
        if any(
            word in content
            for word in ["cloudflare", "checking your browser", "please wait"]
        ):
...
async def main():
    ...
    for url in test_urls:
        ...
        print(f"\n📊 Comparison for {url}:")
        print(
            f"Regular browser content: {len(regular_result.markdown.raw_markdown)} chars"
        )
        print(
            f"Undetected browser content: {len(undetected_result.markdown.raw_markdown)} chars"
        )
