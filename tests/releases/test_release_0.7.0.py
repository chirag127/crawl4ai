...
class TestCrawl4AIv070:
    ...
    async def test_max_pages_limit_batch_processing(self):
        ...
        config = CrawlerRunConfig(cache_mode=CacheMode.BYPASS, max_pages=2)
    ...
    async def test_exclude_external_images(self):
        ...
        config = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, exclude_external_images=True
        )
    ...
    async def test_bm25_content_filter_language_parameter(self):
        ...
        content_filter = BM25ContentFilter(
            user_query="test content", language="english", use_stemming=True
        )

        markdown_generator = DefaultMarkdownGenerator(content_filter=content_filter)

        config = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, markdown_generator=markdown_generator
        )
    ...
    async def test_max_scroll_steps(self):
        ...
        config = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, scan_full_page=True, max_scroll_steps=3
        )
    ...
    async def test_async_url_seeder(self):
        ...
        seeder = AsyncUrlSeeder(base_url="https://httpbin.org", max_depth=1, max_urls=5)
    ...
    async def test_pdf_processing_timeout(self):
        ...
        config = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, pdf=True, pdf_timeout=10000  # 10 seconds
        )
    ...
    async def test_browser_session_management(self):
        ...
        browser_config = BrowserConfig(headless=True, use_persistent_context=True)
    ...
    async def test_virtual_scroll_support(self):
        ...
        config = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, scan_full_page=True, virtual_scroll=True
        )
    ...
    async def test_adaptive_crawling(self):
        ...
        config = CrawlerRunConfig(cache_mode=CacheMode.BYPASS, adaptive_crawling=True)
