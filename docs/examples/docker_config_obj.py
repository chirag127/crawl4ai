...
async def part1_basic_config():
    ...
    browser_config = BrowserConfig(
        headless=False, viewport_width=500, headers={"User-Agent": "Mozilla/5.0"}
    )
...
async def part5_direct_api():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        token_response = await session.post(
            "http://localhost:8000/token", json={"email": "user@example.com"}
        )
...
async def part6_wrap_up():
    ...
    console.print(
        "- **Configurations:** Use the type-params pattern to define settings flexibly."
    )
...
async def main():
    ...
    console.print(
        "[italic]Learn how to work with configuration objects and the Docker API[/italic]\n"
    )
