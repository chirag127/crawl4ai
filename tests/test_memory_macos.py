...
from crawl4ai.utils import (get_memory_stats, get_true_available_memory_gb,
...
def test_memory_calculation():
    ...
    print(
        f"  - Difference: {true_available_gb - psutil_available_gb:.2f} GB of reclaimable memory"
    )
    print()

    # Show the impact on dispatcher behavior
    print("Impact on MemoryAdaptiveDispatcher:")
    thresholds = {"Normal": 90.0, "Critical": 95.0, "Recovery": 85.0}
    ...
    for name, threshold in thresholds.items():
        ...
        if psutil_triggered != true_triggered:
            print(
                f"    → Difference: Platform-aware prevents false {'pressure' if psutil_triggered else 'recovery'}"
            )
    print()
    ...
    for i in range(10):
        ...
        print(
            f"  {i+1}s - PSUtil: {vm.percent:.1f}% | Platform-Aware: {true_pct:.1f}%",
            end="\r",
        )
        time.sleep(1)
    print("\n")
