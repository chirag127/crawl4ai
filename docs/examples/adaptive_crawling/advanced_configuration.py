...
async def main():
    ...
    async with AsyncWebCrawler(verbose=False) as crawler:
        ...
        for config_name, config in [
            ...
            result = await adaptive.digest(
                start_url="https://httpbin.org", query="http headers authentication"
            )
    ...
    async with AsyncWebCrawler(verbose=False) as crawler:
        for strategy_name, config in [
            ...
            result = await adaptive.digest(
                start_url="https://httpbin.org", query="api endpoints"
            )

            # Analyze crawl pattern
            print(f"  - Total pages: {len(result.crawled_urls)}")
            print(
                f"  - Unique domains: {len(set(url.split('/')[2] for url in result.crawled_urls))}"
            )
            print(
                f"  - Max depth reached: {max(url.count('/') for url in result.crawled_urls) - 2}"
            )
            ...
            if hasattr(result, "new_terms_history") and result.new_terms_history:
                ...
                print(
                    f"  - Saturation trend: {'decreasing' if result.new_terms_history[-1] < result.new_terms_history[0] else 'increasing'}"
                )
    ...
    monitor_config = AdaptiveConfig(
        confidence_threshold=0.75, max_pages=10, top_k_links=3
    )
