...
from crawl4ai.browser import (BrowserManager, DockerConfig, DockerRegistry,
                              DockerUtils)
...
async def test_docker_components():
    ...
    try:
        ...
        if not connect_image:
            ...
            return False
        logger.success(
            f"Successfully built connect image: {connect_image}", tag="SETUP"
        )
...
async def test_docker_connect_mode():
    ...
    try:
        ...
        browser_config = BrowserConfig(
            browser_mode="docker", headless=True, docker_config=docker_config
        )
...
async def test_docker_launch_mode():
    ...
    try:
        # Create Docker configuration
        docker_config = DockerConfig(
            mode="launch", persistent=False, remove_on_exit=True, user_data_dir=temp_dir
        )
...
async def test_docker_persistent_storage():
    ...
    temp_dir = os.path.join(
        os.path.dirname(__file__), f"tmp_user_data_persist_{test_id}"
    )
    os.makedirs(temp_dir, exist_ok=True)
    ...
    try:
        ...
        if value == test_value:
            ...
            logger.error(
                f"Storage persistence failed! Expected {test_value}, got {value}",
                tag="TEST",
            )
...
async def test_docker_parallel_pages():
    ...
    try:
        # Create Docker configuration
        docker_config = DockerConfig(
            mode="connect", persistent=False, remove_on_exit=True
        )
...
async def test_docker_registry_reuse():
    ...
    try:
        ...
        browser_config1 = BrowserConfig(
            browser_mode="docker", headless=True, docker_config=docker_config1
        )
        ...
        docker_config2 = DockerConfig(
            mode="connect", persistent=True, registry_file=registry_file
        )

        # Create second browser configuration
        browser_config2 = BrowserConfig(
            browser_mode="docker", headless=True, docker_config=docker_config2
        )
        ...
        if container_id1 == container_id2:
            logger.success(
                "Container reuse successful - using same container!", tag="TEST"
            )
        else:
