...
""",
    ...
    if response.status_code == 200:
        ...
        if "hooks" in data:
            ...
            print(
                f"  Attached hooks: {', '.join(hooks_info['status']['attached_hooks'])}"
            )
            ...
            if hooks_info["execution_log"]:
                ...
                for log_entry in hooks_info["execution_log"]:
                    ...
                    print(
                        f"  {status_icon} {log_entry['hook_point']}: {log_entry['status']} ({log_entry.get('execution_time', 0):.2f}s)"
                    )
...
""",
    ...
    payload = {
        "urls": ["https://httpbin.org/html"],
        "hooks": {"code": hooks_code, "timeout": 5},
    }
        ...
        if data.get("success"):
            ...
            if "hooks" in data:
                ...
                if hooks_info.get("summary", {}).get("successful", 0) > 0:
                    print(
                        f"✅ Authentication hooks executed: {hooks_info['summary']['successful']} successful"
                    )
...
def test_streaming_with_hooks():
    ...
    hooks_code = {"before_retrieve_html": """
async def hook(page, context, **kwargs):
...
"""}
    ...
    with requests.post(
        f"{API_BASE_URL}/crawl/stream", json=payload, stream=True
    ) as response:
        if response.status_code == 200:
...
def test_basic_without_hooks():
    ...
    payload = {"urls": ["https://httpbin.org/html", "https://httpbin.org/json"]}
