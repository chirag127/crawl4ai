...
def load_config() -> Dict:
    ...
    if redis_task_ttl:
        try:
            ...
            logging.info(
                f"Redis task TTL overridden from REDIS_TASK_TTL: {redis_task_ttl}s"
            )
        except ValueError:
            logging.warning(
                f"Invalid REDIS_TASK_TTL value: {redis_task_ttl}, using default"
            )
    ...
    if provider:
        ...
        if provider_temp:
            try:
                ...
                logging.warning(
                    f"Invalid temperature value for {provider_name}: {provider_temp}"
                )
