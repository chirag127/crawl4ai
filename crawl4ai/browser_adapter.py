...
class BrowserAdapter(ABC):
    ...
    @abstractmethod
    async def setup_console_capture(
        self, page: Page, captured_console: List[Dict]
    ) -> Optional[Callable]:
        """Setup console message capturing, returns handler function if needed"""
        pass

    @abstractmethod
    async def setup_error_capture(
        self, page: Page, captured_console: List[Dict]
    ) -> Optional[Callable]:
    ...
    @abstractmethod
    async def cleanup_console_capture(
        self,
        page: Page,
        handle_console: Optional[Callable],
...
class UndetectedAdapter(BrowserAdapter):
    ...
    async def evaluate(
        ...
        isolated = not (
            "__console" in expression
            or "__captured" in expression
            or "__error" in expression
            or "window.__" in expression
        )
    ...
    async def setup_console_capture(
        self, page: UndetectedPage, captured_console: List[Dict]
    ) -> Optional[Callable]:
