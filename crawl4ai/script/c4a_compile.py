...
class C4ACompiler:
    ...
    @classmethod
    def compile(
        cls, script: Union[str, List[str]], root: Optional[pathlib.Path] = None
    ) -> CompilationResult:
        """
        ...
        try:
            ...
            return CompilationResult(
                success=False, errors=[error], metadata={"lineCount": len(script_lines)}
            )
    ...
    def validate(cls, script: Union[str, List[str]]) -> ValidationResult:
        ...
        return ValidationResult(
            valid=result.success, errors=result.errors, warnings=result.warnings
        )
    ...
    def _exception_to_error(
        cls, exc: Exception, script_lines: List[str]
    ) -> ErrorDetail:
    ...
    def _handle_unexpected_token(
        cls, exc: UnexpectedToken, script_lines: List[str]
    ) -> ErrorDetail:
        ...
        line_before = (
            script_lines[line - 2]
            if line > 1 and line <= len(script_lines) + 1
            else None
        )
        ...
        if exc.token.type == "CLICK" and "THEN" in str(exc.expected):
            ...
            suggestions = [Suggestion("Add comma between arguments")]
        else:
            ...
            if exc.token.type == "NAME" and "BACKTICK_STRING" in str(exc.expected):
                ...
                suggestions = [
                    Suggestion("Wrap the selector in backticks", f"`{exc.token.value}`")
                ]
            else:
                code = cls.ERROR_CODES["syntax_error"]
                ...
                if exc.expected:
                    expected_list = [
                        str(e) for e in exc.expected if not str(e).startswith("_")
                    ][:3]
                    if expected_list:
    ...
    def _handle_unexpected_chars(
        cls, exc: UnexpectedCharacters, script_lines: List[str]
    ) -> ErrorDetail:
    ...
    def _handle_value_error(
        cls, exc: ValueError, script_lines: List[str]
    ) -> ErrorDetail:
    ...
    def generate_script(
        ...
        **completion_kwargs,
        ...
        full_prompt = (
            f"{GENERATE_SCRIPT_PROMPT}\n\n{user_prompt}"
            if mode == "c4a"
            else f"{GENERATE_JS_SCRIPT_PROMPT}\n\n{user_prompt}"
        )
        ...
        clean = re.sub(
            r"^```(?:[a-zA-Z0-9_-]+)?\s*|```$", "", raw_response, flags=re.MULTILINE
        ).strip()
...
def compile(
    script: Union[str, List[str]], root: Optional[pathlib.Path] = None
) -> CompilationResult:
    """Compile C4A-Script to JavaScript"""
