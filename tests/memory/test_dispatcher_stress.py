...
class MemorySimulator:
    ...
    def apply_pressure(self, additional_percent: float = 0.0):
        ...
        try:
            logger.info(
                f"Allocating {chunk_size / (1024 * 1024):.1f}MB to reach target memory usage"
            )
    ...
    def release_pressure(self, percent: float = None):
        ...
        if percent is None:
            ...
            if blocks_to_release > 0:
                logger.info(
                    f"Releasing {blocks_to_release} of {len(self.memory_blocks)} memory blocks ({percent}%)"
                )
    ...
    async def _delayed_release(self, delay: float, target_blocks: int):
        ...
        if len(self.memory_blocks) > target_blocks:
            logger.info(
                f"Releasing memory spike ({len(self.memory_blocks) - target_blocks} blocks)"
            )
...
class TestResults:
    ...
    def log_summary(self):
        ...
        retried_urls = {
            url: count for url, count in self.url_to_attempt.items() if count > 1
        }
        if retried_urls:
            logger.info(f"URLs with retries: {len(retried_urls)}")
            # Log the top 5 most retried
            top_retries = sorted(
                retried_urls.items(), key=lambda x: x[1], reverse=True
            )[:5]
...
class StressTestMonitor(CrawlerMonitor):
    ...
    def update_memory_status(self, status: str):
        ...
        self.test_results.max_memory_usage = max(
            self.test_results.max_memory_usage, current_memory
        )
    ...
    def update_queue_statistics(
        self, total_queued: int, highest_wait_time: float, avg_wait_time: float
    ):
        # Track queue metrics in test results
        self.test_results.max_queue_size = max(
            self.test_results.max_queue_size, total_queued
        )
        self.test_results.max_wait_time = max(
            self.test_results.max_wait_time, highest_wait_time
        )
    ...
    def update_task(self, task_id: str, **kwargs):
        ...
        if task_id in self.stats:
            ...
            if "error_message" in kwargs and "requeued" in kwargs["error_message"]:
                if (
                    not hasattr(self.stats[task_id], "counted_requeue")
                    or not self.stats[task_id].counted_requeue
                ):
...
async def run_memory_stress_test(
    ...
    memory_simulator = MemorySimulator(
        target_percent=target_memory_percent, aggressive=aggressive
    )

    logger.info(
        f"Starting stress test with {url_count} URLs in {'STREAM' if STREAM else 'NON-STREAM'} mode"
    )
    ...
    url_chunks = [
        all_urls[i : i + chunk_size] for i in range(0, len(all_urls), chunk_size)
    ]
    ...
    dispatcher = MemoryAdaptiveDispatcher(
        ...
        max_session_permit=(
            20 if aggressive else 10
        ),  # Double the concurrent sessions - pure chaos
    ...
    try:
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            for chunk_index, url_chunk in enumerate(url_chunks):
                logger.info(
                    f"Processing chunk {chunk_index+1}/{len(url_chunks)} ({len(url_chunk)} URLs)"
                )
                ...
                if spikes and chunk_index in spike_intervals:
                    logger.info(
                        f"⚠️ CREATING MASSIVE MEMORY SPIKE at chunk {chunk_index+1} ⚠️"
                    )
                    ...
                    if random.random() < 0.5:
                        ...
                        memory_simulator.spike_pressure(
                            duration=8.0
                        )  # 8-second overlapping spike
                ...
                if chunk_index % 5 == 4:  # Less frequent releases
                    release_percent = random.choice(
                        [10, 15, 20]
                    )  # Smaller, inconsistent releases
                    logger.info(
                        f"Releasing {release_percent}% of memory blocks - brief respite"
                    )
        ...
        if len(test_results.completed_urls) + len(test_results.failed_urls) < url_count:
            logger.error(
                f"TEST FAILED: Not all URLs were processed. {url_count - len(test_results.completed_urls) - len(test_results.failed_urls)} URLs missing."
            )
            return False
