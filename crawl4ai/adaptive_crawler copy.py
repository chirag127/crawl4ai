...
class CrawlState:
    ...
    def save(self, path: Union[str, Path]):
        ...
        state_dict = {
            ...
            "kb_embeddings": (
                self.kb_embeddings.tolist() if self.kb_embeddings is not None else None
            ),
            "query_embeddings": (
                self.query_embeddings.tolist()
...
class AdaptiveConfig:
    ...
    embedding_k_exp: float = (
        3.0  # Exponential decay factor for distance-to-score mapping
    )
    ...
    embedding_validation_min_score: float = (
        0.4  # Minimum validation score to trust convergence
    )
...
class EmbeddingStrategy(CrawlStrategy):
    ...
    async def map_query_semantic_space(self, query: str, n_synthetic: int = 10) -> Any:
        ...
        provider = (
            self.llm_config.get("provider", "openai/gpt-4o-mini")
            if self.llm_config
        ...
        variations = {
            "queries": [
                ...
                "can you provide a quick recipe for vegetable fried rice?",
                "what cooking techniques are essential for perfect fried rice with vegetables?",
...
class AdaptiveCrawler:
    ...
    async def digest(
        ...
        if isinstance(self.strategy, EmbeddingStrategy) and not resume_from:
            ...
            query_embeddings, expanded_queries = (
    ...
    def _crawl_result_to_export_dict(self, result) -> Dict[str, Any]:
        ...
        if self.state:
            export_dict["crawl_metadata"] = {
                "crawl_order": (
                    self.state.crawl_order.index(export_dict["url"]) + 1
