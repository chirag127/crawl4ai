...
async def test_basic_progressive_crawl():
    ...
    config = AdaptiveConfig(
        confidence_threshold=0.7, max_pages=10, top_k_links=2, min_gain_threshold=0.1
    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        console.print(
            f"URLs crawled: {list(state.crawled_urls)[:5]}..."
        )  # Show first 5
...
async def test_with_persistence():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        state1 = await prog_crawler.digest(
            start_url="https://httpbin.org", query="http headers response"
        )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        console.print(
            f"[green]Resumed crawl: {len(state2.crawled_urls)} total pages[/green]"
        )
...
async def test_different_domains():
    ...
    for test in test_cases:
        ...
        config = AdaptiveConfig(confidence_threshold=0.6, max_pages=5, top_k_links=2)
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            start_time = time.time()
            state = await prog_crawler.digest(
                start_url=test["url"], query=test["query"]
            )
            elapsed = time.time() - start_time
...
async def test_stopping_criteria():
    ...
    config = AdaptiveConfig(
        confidence_threshold=0.9, max_pages=20, top_k_links=3  # Very high
    )
    ...
    config = AdaptiveConfig(
        confidence_threshold=0.9, max_pages=3, top_k_links=2  # Very low limit
    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        console.print(
            f"Stopped by: {'Page limit' if len(state.crawled_urls) >= 3 else 'Other'}"
        )
        prog_crawler.print_stats(detailed=False)
...
async def test_crawl_patterns():
    ...
    config = AdaptiveConfig(
        confidence_threshold=0.7, max_pages=8, top_k_links=2, min_gain_threshold=0.05
    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        state = await prog_crawler.digest(
            start_url="https://httpbin.org", query="http methods post get"
        )
