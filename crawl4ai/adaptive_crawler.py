...
class AdaptiveConfig:
    ...
    def validate(self):
        ...
        assert (
            0 <= self.embedding_min_confidence_threshold <= 1
...
class EmbeddingStrategy(CrawlStrategy):
    ...
    async def map_query_semantic_space(self, query: str, n_synthetic: int = 10) -> Any:
        ...
        response = perform_completion_with_backoff(
            ...
            base_url=base_url,
            base_delay=(
                llm_config_dict.get("backoff_base_delay", 2) if llm_config_dict else 2
            ),
            max_attempts=(
                llm_config_dict.get("backoff_max_attempts", 3) if llm_config_dict else 3
            ),
            exponential_factor=(
                llm_config_dict.get("backoff_exponential_factor", 2)
                if llm_config_dict
                else 2
            ),
        )
