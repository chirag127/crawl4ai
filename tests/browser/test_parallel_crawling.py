...
async def test_parallel_approaches_comparison():
    ...
    try:
        ...
        logger.info(
            f"Approach 1 time (get_page + gather): {approach1_time:.2f}s", tag="TEST"
        )
        ...
        tasks = [
            fetch_title_approach2(page_ctx, url) for page_ctx, url in zip(pages, urls)
        ]
        approach2_results = await asyncio.gather(*tasks)

        approach2_time = time.time() - start_time
        logger.info(
            f"Approach 2 time (get_pages + gather): {approach2_time:.2f}s", tag="TEST"
        )
        ...
        if speedup > 1:
            logger.success(
                f"Approach 2 (get_pages upfront) was {speedup:.2f}x faster", tag="TEST"
            )
        else:
            logger.info(
                f"Approach 1 (get_page + gather) was {1/speedup:.2f}x faster",
                tag="TEST",
            )
        ...
        assert sorted(approach1_results) == sorted(
            approach2_results
        ), "Results content mismatch"
...
async def test_multi_browser_scaling(num_browsers=3, pages_per_browser=5):
    ...
    """
    logger.info(
        f"Testing multi-browser scaling with {num_browsers} browsers × {pages_per_browser} pages",
        tag="TEST",
    )
    ...
    try:
        ...
        logger.info(
            f"📊 Summary: {num_browsers} browsers × {pages_per_browser} pages = {total_pages} total crawls",
            tag="TEST",
        )
        logger.info(
            f"⚡ Performance: {pages_per_second:.1f} pages/second ({pages_per_second*60:.0f} pages/minute)",
            tag="TEST",
        )
        ...
        if speedup > 1:
            logger.success(
                f"✅ Approach 2 (get_pages upfront) was {speedup:.2f}x faster",
                tag="TEST",
            )
        else:
            logger.info(
...
async def grid_search_optimal_configuration(total_urls=50):
    ...
    """
    logger.info(
        f"=== GRID SEARCH FOR OPTIMAL CRAWLING CONFIGURATION ({total_urls} URLs) ===",
        tag="TEST",
    )
    ...
    for num_browsers in range(1, max_browsers_to_test + 1):
        ...
        if remainder > 0:
            ...
            page_distribution = [base_pages_per_browser + 1] * remainder + [
                base_pages_per_browser
            ] * (num_browsers - remainder)
            ...
            pages_distribution = (
                f"{base_pages_per_browser} pages × {num_browsers} browsers"
            )
    ...
    for (
        ...
        logger.info(
            f"Testing configuration: {num_browsers} browsers with distribution: {distribution_str}",
            tag="TEST",
        )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "psutil not available, memory metrics will not be tracked",
                    tag="TEST",
                )
                initial_memory = 0
            ...
            result = {
                ...
                "efficiency_score": (
                    pages_per_second / (peak_memory + 1)
                    if peak_memory > 0
                    else pages_per_second
                ),
            }
            ...
            if peak_memory > 0:
                ...
                logger.info(
                    f"Efficiency score: {result['efficiency_score']:.6f}", tag="TEST"
                )
    ...
    lowest_memory = sorted(
        results,
        key=lambda x: x["peak_memory"] if x["peak_memory"] > 0 else float("inf"),
    )[0]
    ...
    logger.info(
        f"⚡ Fastest: {fastest['num_browsers']} browsers × ~{fastest['pages_per_browser']} pages "
        + f"({fastest['crawl_time']:.2f}s, {fastest['pages_per_second']:.1f} pages/s)",
        tag="TEST",
    )

    if lowest_memory["peak_memory"] > 0:
        logger.info(
            f"💾 Lowest memory: {lowest_memory['num_browsers']} browsers × ~{lowest_memory['pages_per_browser']} pages "
            + f"({lowest_memory['peak_memory']:.1f}MB)",
            tag="TEST",
        )

    logger.info(
        f"🌟 Most efficient: {most_efficient['num_browsers']} browsers × ~{most_efficient['pages_per_browser']} pages "
        + f"(score: {most_efficient['efficiency_score']:.6f})",
        tag="TEST",
    )
    ...
    for rank, result in enumerate(ranked_results, 1):
        ...
        memory_str = (
            f"{result['peak_memory']:.1f}" if result["peak_memory"] > 0 else "N/A"
        )

        # Get the distribution string
        dist_str = result.get(
            "distribution_str",
            str(tuple([result["pages_per_browser"]] * result["num_browsers"])),
        )
    ...
    try:
        ...
        for bar, score in zip(bars, efficiency_scores):
            ...
            ax1.text(
                ...
                height + 0.02 * max(efficiency_scores),
                f"{score:.3f}",
                ha="center",
                va="bottom",
                rotation=90,
        ...
        ax1.text(
            0.02,
            0.90,
            f"🌟 Most Efficient: {most_efficient['num_browsers']} browsers with ~{most_efficient['pages_per_browser']} pages",
        ...
        ax2.plot(
            browser_counts, crawl_times, "bo-", label="Crawl Time (s)", linewidth=2
        )
        ax2.plot(
            browser_counts,
            total_times,
        ...
        ax2.plot(
            browser_counts[fastest_idx],
            crawl_times[fastest_idx],
            "ro",
            ms=10,
        ...
        ax2_twin = ax2.twinx()
        ax2_twin.plot(
            browser_counts, pages_per_second, "r^--", label="Pages/second", alpha=0.5
        )
        ...
        ax2.text(
            0.02,
            0.90,
            f"⚡ Fastest: {fastest['num_browsers']} browsers with ~{fastest['pages_per_browser']} pages"
        ...
        if memory_results:
            ...
            memory_per_browser = [
                m / n for m, n in zip(peak_memories, memory_browser_counts)
            ]
            memory_per_page = [
                m / (n * p)
                for m, n, p in zip(
            ...
            lowest_memory_idx = memory_browser_counts.index(
                lowest_memory["num_browsers"]
            )
            ...
            ax3.text(
                0.02,
                0.90,
                f"💾 Lowest Memory: {lowest_memory['num_browsers']} browsers with ~{lowest_memory['pages_per_browser']} pages"
        ...
        # Add overall title
        plt.suptitle(
            f"Browser Scaling Grid Search Results for {total_urls} URLs",
            fontsize=16,
            y=0.98,
        )
        ...
        plt.figtext(
            0.5,
            0.01,
            f"Generated by Crawl4AI at {time.strftime('%Y-%m-%d %H:%M:%S')}",
            ha="center",
            fontsize=10,
            style="italic",
        )
...
async def find_optimal_browser_config(
    total_urls=50, verbose=True, rate_limit_delay=0.2
):
    ...
    for num_browsers in range(1, max_browsers + 1):
        ...
        if remainder > 0:
            distribution = [base_pages + 1] * remainder + [base_pages] * (
                num_browsers - remainder
            )
        else:
    ...
    for browser_count, page_distribution in configs_to_test:
        if verbose:
            print(
                f"Testing {browser_count} browsers with distribution {tuple(page_distribution)}"
            )
        ...
        try:
            ...
            for manager, manager_urls in urls_per_manager.items():
                ...
                pages = await manager.get_pages(
                    CrawlerRunConfig(), count=len(manager_urls)
                )
                all_pages.extend(zip(pages, manager_urls))
            ...
            if verbose:
                print(
                    f"  ✓ Crawled {total_urls} pages in {crawl_time:.2f}s ({pages_per_second:.1f} pages/sec)"
                )
                if memory_used > 0:
                    print(
                        f"  ✓ Memory used: {memory_used:.1f}MB ({memory_used/total_urls:.1f}MB per page)"
                    )
    ...
    if verbose:
        ...
        print(
            f"⚡ Fastest: {fastest['browser_count']} browsers {fastest['distribution']}"
        )
        print(
            f"   {fastest['crawl_time']:.2f}s, {fastest['pages_per_second']:.1f} pages/sec"
        )

        print(
            f"💾 Memory-efficient: {lowest_memory['browser_count']} browsers {lowest_memory['distribution']}"
        )
        if lowest_memory["memory_used"] > 0:
            print(
                f"   {lowest_memory['memory_used']:.1f}MB, {lowest_memory['memory_used']/total_urls:.2f}MB per page"
            )

        print(
            f"🌟 Balanced optimal: {optimal['browser_count']} browsers {optimal['distribution']}"
        )
        print(
            f"   {optimal['crawl_time']:.2f}s, {optimal['pages_per_second']:.1f} pages/sec, score: {optimal['efficiency']:.4f}"
        )
...
async def run_tests():
    ...
    if configs:
        ...
        print(
            f"   {optimal['browser_count']} browsers with distribution {optimal['distribution']}"
        )
        print(
            f"   Estimated performance: {optimal['pages_per_second']:.1f} pages/second"
        )
        results.append(True)
    else:
