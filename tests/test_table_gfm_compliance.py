...
class TestTableGFMCompliance:
    ...
    def test_table_has_leading_pipes(self):
        ...
        for i, line in enumerate(lines):
            assert line.startswith(
                "|"
            ), f"Line {i+1} missing leading pipe: {repr(line)}"
            ...
            for cell in cells[1:-1]:
                if cell.strip():  # Non-empty cell
                    assert cell.startswith(
                        " "
                    ), f"Cell missing leading space: {repr(line)}"
                    assert cell.endswith(
                        " "
                    ), f"Cell missing trailing space: {repr(line)}"
    ...
    def test_table_starts_on_own_line(self):
        ...
        for line in result.split("\n"):
            ...
            if stripped.startswith("|"):
                ...
                assert line.startswith(
                    "|"
                ), f"Table row not at line start: {repr(line)}"
                break
        else:
    ...
    def test_caption_does_not_merge_with_header(self):
        ...
        assert (
            len(table_lines) >= 3
        ), f"Expected at least 3 table rows, got {len(table_lines)}"
        assert table_lines[0].startswith(
            "|"
        ), f"Header not starting with pipe: {repr(table_lines[0])}"
...
class TestPadTablesUnchanged:
    ...
    def test_pad_tables_produces_aligned_output(self):
        ...
        for line in lines:
            assert line.startswith(
                "|"
            ), f"Padded row missing leading pipe: {repr(line)}"
