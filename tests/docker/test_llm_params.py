...
def print_response(response: dict, title: str = "Response"):
    ...
    if "markdown" in response:
        relevant["markdown"] = (
            response["markdown"][:200] + "..."
            if len(response.get("markdown", "")) > 200
...
async def test_default_no_params(client: httpx.AsyncClient):
    ...
    try:
        ...
        console.print(
            "[dim]→ This used system defaults or environment variables if set[/]"
        )
    except Exception as e:
...
async def test_request_temperature(client: httpx.AsyncClient):
    ...
    try:
        ...
        console.print(
            "[dim]→ Low temperature (0.1) should produce focused, less creative output[/]"
        )
    except Exception as e:
        console.print(f"[red]Error: {e}[/]")
    ...
    try:
        ...
        console.print(
            "[dim]→ High temperature (1.5) should produce more creative, varied output[/]"
        )
    except Exception as e:
        console.print(f"[red]Error: {e}[/]")
...
async def test_base_url_custom(client: httpx.AsyncClient):
    ...
    console.print(
        "[yellow]Note: This will fail unless you have a custom endpoint set up[/]"
    )
    ...
    try:
        ...
        console.print(
            f"[yellow]Expected failure (no custom endpoint): Status {e.response.status_code}[/]"
        )
    except Exception as e:
    ...
    try:
        ...
        if "task_id" in job_data:
            ...
            if status_data.get("status") == "completed":
                ...
                console.print(
                    f"[yellow]Job status: {status_data.get('status', 'unknown')}[/]"
                )
        else:
...
async def show_environment_info():
    ...
    table = Table(
        title="LLM Environment Variables", show_header=True, header_style="bold magenta"
    )
...
async def main():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60.0) as client:
        ...
        if not await check_server_health(client):
            console.print(
                "[red]Server is not available. Please ensure the Docker container is running.[/]"
            )
            return
        ...
        console.print(
            "1. [yellow]Request parameters[/] - Highest priority (temperature, base_url in API call)"
        )
        console.print(
            "2. [yellow]Provider-specific env[/] - e.g., OPENAI_TEMPERATURE, GROQ_BASE_URL"
        )
        console.print(
            "3. [yellow]Global env variables[/] - LLM_TEMPERATURE, LLM_BASE_URL"
        )
        console.print(
            "4. [yellow]System defaults[/] - Lowest priority (provider/litellm defaults)"
        )
        console.print()
