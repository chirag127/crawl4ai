...
from crawl4ai import (AsyncWebCrawler, BrowserConfig, CacheMode,
                      CrawlerRunConfig, JsonCssExtractionStrategy, LLMConfig,
...
class ModelConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"provider": self.provider, "api_token": self.api_token}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ModelConfig":
        return cls(provider=data["provider"], api_token=data["api_token"])
...
class WebScraperAgent:
    ...
    async def _load_or_generate_schema(
        ...
        url: str,
        query: str,
        session_id: str = "schema_generator",
        ...
        if model_name:
            ...
            if model_config:
                llm_config = LLMConfig(
                    provider=model_config.provider, api_token=model_config.api_token
                )
                print(f"Using custom model: {model_name}")
            else:
                raise ValueError(
                    f"Model configuration '{model_name}' not found. Please add it from the Models page."
                )
        else:
            # Require a model to be specified
            raise ValueError(
                "No model specified. Please select a model from the dropdown or add one from the Models page."
            )

        schema = JsonCssExtractionStrategy.generate_schema(
            html=html, llm_config=llm_config, query=query
        )
    ...
    async def scrape_data_with_llm(
        self, url: str, query: str, model_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        llm_extraction_strategy = LLMExtractionStrategy(
            llm_config=llm_config, instruction=query, result_type="json", schema=schema
        )
    ...
    async def scrape_data(
        ...
        schema = await self._load_or_generate_schema(
            url=url, query=query, model_name=model_name
        )
...
async def scrape_website(
    url: str, query: str, model_name: Optional[str] = None
) -> Dict[str, Any]:
    """
...
async def scrape_website_with_llm(
    url: str, query: str, model_name: Optional[str] = None
):
    """
    Scrape structured data from any website using a custom LLM model.
