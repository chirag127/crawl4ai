...
async def main():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        new_config = crawler_config.clone(
            js_code=["(() => ({'data':'hello'}))()"], js_only=True
        )
        result: CrawlResult = (
            await crawler.arun(  # This time there is no fetch and this only executes JS in the same opened page
                url="https://www.helloworld.org", config=new_config
            )
        )
        print(
            result.js_execution_result
        )  # You should see {'data':'hello'} in the console
