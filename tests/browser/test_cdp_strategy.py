...
async def test_cdp_launch_connect():
    ...
    browser_config = BrowserConfig(
        browser_mode="cdp", use_managed_browser=True, headless=True
    )
...
async def test_cdp_with_user_data_dir():
    ...
    browser_config = BrowserConfig(
        headless=True, use_managed_browser=True, user_data_dir=user_data_dir
    )
    ...
    try:
        ...
        has_test_cookie = any(
            cookie["name"] in ["test_cookie", "test_cookie_api"] for cookie in cookies
        )
        ...
        logger.info(
            "Starting second browser session with same user data directory", tag="TEST"
        )
        ...
        has_test_cookie2 = any(
            cookie["name"] in ["test_cookie", "test_cookie_api"] for cookie in cookies2
        )
...
async def test_cdp_session_management():
    ...
    browser_config = BrowserConfig(use_managed_browser=True, headless=True)
    ...
    try:
        ...
        logger.info(
            f"Session 1 setup - URL: {page1_url}, Title: {page1_title}", tag="TEST"
        )
        ...
        logger.info(
            f"Session 2 setup - URL: {page2_url}, Title: {page2_title}", tag="TEST"
        )
        ...
        logger.info(
            f"Sessions in manager - S1: {session1_exists}, S2: {session2_exists}",
            tag="TEST",
        )
        ...
        logger.info(
            f"Session 1 reuse - Same page: {is_same_page}, Same context: {is_same_context}",
            tag="TEST",
        )
        ...
        logger.info(
            f"Session tracking - S1 complete: {session1_has_timestamp}, S2 complete: {session2_has_timestamp}",
            tag="TEST",
        )
        ...
        logger.info(
            f"Sessions have different timestamps: {sessions_have_different_timestamps}",
            tag="TEST",
        )
        ...
        logger.info(
            f"After kill - S1 removed: {session1_removed}, S2 exists: {session2_still_exists}",
            tag="TEST",
        )
        ...
        success = (
            ...
            and session2_exists
            and is_same_page
            and session1_has_timestamp
            and session2_has_timestamp
            and sessions_have_different_timestamps
            and session1_removed
            and session2_removed
        )
...
async def test_cdp_timing_fix_fast_startup():
    ...
    try:
        ...
        await manager.close()
        logger.success(
            "test_cdp_timing_fix_fast_startup completed successfully", tag="TEST"
        )
        return True
...
async def test_cdp_timing_fix_delayed_browser_start():
    ...
    try:
        ...
        logger.info(
            f"BrowserManager connected successfully in {startup_time:.2f}s", tag="TEST"
        )
        ...
        logger.success(
            "test_cdp_timing_fix_delayed_browser_start completed successfully",
            tag="TEST",
        )
        return True
        ...
        logger.error(
            f"test_cdp_timing_fix_delayed_browser_start failed: {str(e)}", tag="TEST"
        )
        try:
            await manager.close()
            await managed_browser.cleanup()
...
async def test_cdp_verification_backoff_behavior():
    ...
    try:
        ...
        assert (
            result is False
        ), "Expected CDP verification to fail with non-existent endpoint"

        # Should take some time due to retries and backoff
        assert (
            elapsed_time > 2.0
        ), f"Expected backoff delays, but took only {elapsed_time:.2f}s"

        logger.info(
            f"CDP verification correctly failed after {elapsed_time:.2f}s with exponential backoff",
            tag="TEST",
        )
        logger.success(
            "test_cdp_verification_backoff_behavior completed successfully", tag="TEST"
        )
        return True

    except Exception as e:
        logger.error(
            f"test_cdp_verification_backoff_behavior failed: {str(e)}", tag="TEST"
        )
        return False
