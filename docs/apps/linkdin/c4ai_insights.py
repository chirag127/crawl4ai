...
def build_company_graph(companies, embeds: np.ndarray, top_k: int) -> Dict[str, Any]:
    ...
    for i, c in enumerate(companies):
        ...
        for j in top_idx:
            ...
            follower_ratio = min(node["followers"], tgt.get("followers", 1)) / max(
                node["followers"] or 1, tgt.get("followers", 1)
            )
                ...
                dict(
                    ...
                    drivers=dict(
                        ...
                        industry_match=(
                            0.10
                            if node["industry"]
                            == tgt.get("descriptor", "").split("•")[0].strip()
                            else 0
                        ),
...
async def infer_org_chart_llm(
    ...
    if stub:
        ...
        return {
            "nodes": nodes,
            "edges": [],
            "meta": {"debug_stub": True, "generated_at": datetime.now(UTC).isoformat()},
        }
    ...
    prompt = [
        ...
        {
            "role": "user",
            "content": f"""Here is the company description:
...
async def run(opts):
    ...
    console.print(
        f"[bold cyan]Loaded[/] {len(companies)} companies, {len(people)} people"
    )
    ...
    if not to_process:
        ...
        console.print(
            f"[bold]⇢[/] Inferring org-charts via LLM {f'(parallel={workers} workers)' if parallel else ''}"
        )
        ...
        with Progress(
            ...
            async def process_one(comp):
                ...
                persons = [
                    p
                    for p in people
                    if p["company_handle"].strip("/") == comp["handle"].strip("/")
                ]
                ...
                progress.update(
                    task, advance=1, description=f"{comp['name']} ({len(persons)} ppl)"
                )
...
def build_arg_parser():
    p = argparse.ArgumentParser(
        description="Build graphs & visualisation from Stage-1 output"
    )
    p.add_argument(
        "--in", dest="in_dir", required=False, help="Stage-1 output dir", default="."
    )
    p.add_argument(
        "--out", dest="out_dir", required=False, help="Destination dir", default="."
    )
    ...
    p.add_argument(
        "--llm-api-key", help="API key for LLM provider (defaults to env vars)"
    )
    ...
    p.add_argument(
        "--stub",
        action="store_true",
        help="Skip OpenAI call and generate tiny fake org charts",
    )
    p.add_argument(
        "--workers",
        type=int,
