...
class ScoringStats:
    ...
    def get_min(self) -> float:
        ...
        if self._min_score is None:
            self._min_score = (
                self._total_score / self._urls_scored if self._urls_scored else 0.0
            )
...
class KeywordRelevanceScorer(URLScorer):
    ...
    def _url_bytes(self, url: str) -> bytes:
        ...
        return (
            url.encode("utf-8") if self._case_sensitive else url.lower().encode("utf-8")
        )
...
class DomainAuthorityScorer(URLScorer):
    ...
    def __init__(
        ...
        self._top_domains = {
            domain: score
            for domain, score in sorted(domain_weights.items(), key=lambda x: -x[1])[
                :5
            ]  # Keep top 5 highest scoring domains
        }
