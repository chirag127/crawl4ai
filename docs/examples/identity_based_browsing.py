...
async def crawl_with_profile(profile_path, url):
    ...
    logger.info(
        f"\nCrawling {Fore.CYAN}{url}{Style.RESET_ALL} using profile at {Fore.YELLOW}{profile_path}{Style.RESET_ALL}",
        tag="CRAWL",
    )
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        if result.success:
            ...
            logger.success(
                f"Page title: {Fore.GREEN}{title}{Style.RESET_ALL}", tag="CRAWL"
            )
            return result
        else:
            # Log error status
