...
def record_result(
    name: str, issue: str, passed: bool, message: str, skipped: bool = False
):
...
async def test_llm_input_format():
    ...
    try:
        ...
        default_strategy = LLMExtractionStrategy(
            llm_config=llm_config, instruction="Extract data"
        )
        ...
        html_strategy = LLMExtractionStrategy(
            llm_config=llm_config, instruction="Extract data", input_format="html"
        )
...
async def test_redirect_url_handling():
    ...
    try:
        ...
        async with AsyncWebCrawler() as crawler:
            result = await crawler.arun(url=test_url, config=CrawlerRunConfig())

        # Verify redirected_url is populated
        if not result.redirected_url:
            record_result(
                "Redirect URL Handling", "#1268", False, "redirected_url is empty"
            )
            return
...
async def test_pypdf_migration():
    ...
    try:
        ...
        try:
            ...
            uses_pypdf = (
                "from pypdf" in processor_source or "import pypdf" in processor_source
            )
            uses_pypdf2 = (
                "from PyPDF2" in processor_source or "import PyPDF2" in processor_source
            )

            if uses_pypdf2 and not uses_pypdf:
                record_result(
                    "pypdf Migration", "#1412", False, "PDF processor still uses PyPDF2"
                )
                return
            ...
            if uses_pypdf:
                ...
                record_result(
                    ...
                    "#1412",
                    True,
                    "PDF processor found, pypdf dependency updated",
                    skipped=not pypdf_available,
                )
                return
            ...
            if pypdf_available:
                ...
                record_result(
                    ...
                    "#1412",
                    True,
                    "PDF support not installed (optional feature)",
                    skipped=True,
                )
            return
...
async def test_pydantic_configdict():
    ...
    try:
        ...
        try:
            ...
            record_result(
                "Pydantic ConfigDict", "#678", False, f"Model instantiation failed: {e}"
            )
...
async def test_docker_content_filter():
    ...
    try:
        ...
        filter_instance = ContentRelevanceFilter(query="test query", threshold=0.3)

        if not hasattr(filter_instance, "query"):
            record_result(
                "Docker ContentRelevanceFilter",
                "#1642",
                False,
...
async def test_docker_cache_permissions():
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                if (
                    ...
                    and "denied" in str(result).lower()
                ):
                    record_result(
                        "Docker Cache Permissions",
                        "#1638",
                        False,
        ...
        record_result(
            ...
            "#1638",
            True,
            f"Skipped - Docker not available: {e}",
            skipped=True,
        )
...
async def test_adaptive_crawler_embedding():
    ...
    try:
        ...
        if "class EmbeddingStrategy" not in source_code:
            record_result(
                "AdaptiveCrawler Query Expansion",
                "#1621",
                True,
        ...
        for i, line in enumerate(lines):
            ...
            if in_embedding_strategy:
                ...
                if (
                    "perform_completion_with_backoff" in line
                    and not line.strip().startswith("#")
                ):
                    found_llm_call = True
                # Check for commented mock data
                if (
                    "variations ={'queries'" in line
                    or "variations = {'queries'" in line
                ):
        ...
        record_result(
            "AdaptiveCrawler Query Expansion", "#1621", False, f"Exception: {e}"
        )
...
def main():
    pass
        ...
        async with AsyncWebCrawler() as crawler:
            result = await crawler.arun(
                url=f"raw:{html_with_code}", config=CrawlerRunConfig()
            )
