    ...
    try:
        async with session.post(url, json=payload) as response:
            ...
            if status == 200:
                ...
                raise Exception(
                    f"Failed to get token: {data.get('detail', 'Unknown error')}"
                )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for url in urls:
            ...
            result = await test_endpoint(
                session, "llm", url, token, {"q": "Extract title and main content"}
            )
        ...
        print("\n=== Testing Error Cases ===")
        # Test invalid URL
        await test_endpoint(session, "md", "not_a_real_url", token, expected_status=500)

        # Test invalid filter type
        await test_endpoint(
            session, "md", "example.com", token, {"f": "invalid"}, expected_status=422
        )

        # Test LLM without query (should fail per your server logic)
        await test_endpoint(session, "llm", "example.com", token, expected_status=400)
