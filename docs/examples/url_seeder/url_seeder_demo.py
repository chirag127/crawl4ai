...
async def section_2_cache_demo(seed: AsyncUrlSeeder):
    ...
    console.print(
        "[yellow]Using `force=True` to bypass cache and fetch fresh data.[/yellow]"
    )
    cfg = SeedingConfig(
        source="cc", pattern="*crawl4ai.com/core/*", verbose=False, force=True
    )
...
async def section_4_bm25_scoring(seed: AsyncUrlSeeder):
    ...
    console.print(
        "[yellow]Using AI-powered relevance scoring to find the most relevant content[/yellow]"
    )
...
async def section_5_keyword_filter_to_agent(seed: AsyncUrlSeeder):
    ...
    selected = [
        u for u in urls if any(k in str(u["head_data"]).lower() for k in keywords)
    ]
    ...
    async with AsyncWebCrawler(verbose=True) as crawler:
        ...
        crawled_results_stream = await crawler.arun_many(
            urls_to_crawl, config=crawl_run_config
        )
        final_crawled_data = []
        async for result in crawled_results_stream:
            ...
            if len(final_crawled_data) % 5 == 0:
                print(
                    f"   Processed {len(final_crawled_data)}/{len(urls_to_crawl)} URLs..."
                )
        ...
        if final_crawled_data:
            ...
            for result in final_crawled_data:
                if result.success and result.markdown.raw_markdown:
                    ...
                    print(
                        f"     Markdown snippet: {result.markdown.raw_markdown[:200]}..."
                    )
                    break
            else:
...
async def section_6_multi_domain(seed: AsyncUrlSeeder):
    ...
    console.print(
        "[yellow]Discovering Python tutorials across multiple educational sites[/yellow]\n"
    )
    ...
    console.print(
        f"[green]✓ Found {total_urls} relevant URLs across {len(domains)} domains in {elapsed:.2f}s\n"
    )
    ...
    for domain, urls in results.items():
        ...
        if urls:
            top = urls[0]
            console.print(
                f"  Top result: [{top['relevance_score']:.2f}] {top['head_data'].get('title', 'No title')}"
            )
...
async def main():
    async with AsyncUrlSeeder() as seed:
        ...
        sections = {
            ...
            "5": (
                "Complete Pipeline (Discover → Filter → Crawl)",
