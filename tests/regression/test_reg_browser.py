...
async def test_wait_for_css_selector(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        assert "Dynamic content successfully loaded" in (
            result.markdown or ""
...
async def test_js_code_modifies_dom(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            result.success
        ), f"JS DOM modification crawl failed: {result.error_message}"
        combined = (result.html or "") + (result.markdown or "")
        assert (
            "Injected by JS" in combined
...
async def test_js_code_returns_value(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        if result.js_execution_result is not None:
            ...
            assert (
                "Crawl4AI Test Home" in result_str or len(result_str) > 0
...
async def test_multiple_js_scripts(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            result.success
        ), f"Multiple JS scripts crawl failed: {result.error_message}"
...
async def test_force_viewport_screenshot(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            result.success
        ), f"Force viewport screenshot crawl failed: {result.error_message}"
...
async def test_remove_overlay_elements(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            len(result.html) > 0
        ), "HTML should still be present after overlay removal"
...
async def test_real_url_with_wait():
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            "quote" in combined.lower() or "quotes" in combined.lower()
...
async def test_real_url_screenshot():
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            result.success
        ), f"Real URL screenshot crawl failed: {result.error_message}"
...
async def test_crawl_empty_page(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        if not result.success:
            assert (
                "empty" in (result.error_message or "").lower()
...
async def test_crawl_malformed_html(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        ), "HTML should not be None even for malformed input"
        # The content is present in the HTML even if the crawl is marked as not successful
        assert "Unclosed paragraph" in (result.html or "") or "Malformed" in (
            result.html or ""
...
async def test_wait_until_networkidle(local_server):
    ...
    async with AsyncWebCrawler(
        ...
        assert (
            result.success
        ), f"wait_until=networkidle crawl failed: {result.error_message}"
