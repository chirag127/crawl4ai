...
from crawl4ai import (AsyncWebCrawler, BrowserConfig, CacheMode,
                      ...
                      DisplayMode, MemoryAdaptiveDispatcher, RateLimiter)
...
class SiteGenerator:
    ...
    def __init__(
        self, site_path: str = DEFAULT_SITE_PATH, page_count: int = DEFAULT_URL_COUNT
    ):
...
"""
    ...
    def generate_site(self) -> None:
        ...
        for i in range(self.page_count):
            paragraphs = "\n".join(
                f"<p>{' '.join(random.choices(self.lorem_words, k=200))}</p>"
                for _ in range(5)
            )
            images = "\n".join(
                f'<img src="https://picsum.photos/seed/{secrets.token_hex(8)}/300/200" loading="lazy" alt="Random image {j}"/>'
                for j in range(3)
            )
            page_path = self.site_path / f"page_{i}.html"
            page_path.write_text(
                self.html_template.format(
                    page_num=i, paragraphs=paragraphs, images=images
                ),
                encoding="utf-8",
            )
        ...
        console.print(
            f"[bold green]Successfully generated {self.page_count} test pages in [cyan]{self.site_path}[/cyan][/bold green]"
        )
...
class LocalHttpServer:
    ...
    def start(self) -> None:
        if not self.site_path.exists():
            raise FileNotFoundError(f"Site directory {self.site_path} does not exist")
        console.print(
            f"Attempting to start HTTP server in [cyan]{self.site_path}[/cyan] on port {self.port}..."
        )
        try:
            ...
            if self.is_running():
                console.print(
                    f"[bold green]HTTP server started successfully (PID: {self.process.pid})[/bold green]"
                )
            else:
                console.print(
                    "[bold red]Failed to start HTTP server. Checking logs...[/bold red]"
                )
            ...
            console.print(f"[bold red]Error starting HTTP server: {str(e)}[/bold red]")
            self.stop()
            raise
        ...
        if self.process and self.is_running():
            ...
            try:
                ...
                try:
                    ...
                    console.print("[bold yellow]HTTP server stopped[/bold yellow]")
                except subprocess.TimeoutExpired:
                    console.print(
                        "[bold red]Server did not terminate gracefully, killing...[/bold red]"
                    )
                    self.process.kill()
                    stdout, stderr = self.process.communicate()
                    console.print("[bold yellow]HTTP server killed[/bold yellow]")
            except Exception as e:
                console.print(
                    f"[bold red]Error stopping HTTP server: {str(e)}[/bold red]"
                )
                self.process.kill()
            finally:
                self.process = None
        elif self.process:
            console.print("[dim]HTTP server process already stopped.[/dim]")
            self.process = None
...
class SimpleMemoryTracker:
    ...
    def __init__(
        self, report_path: str = DEFAULT_REPORT_PATH, test_id: Optional[str] = None
    ):
        ...
        with open(self.csv_path, "w", encoding="utf-8") as f:
            f.write("timestamp,elapsed_seconds,memory_info_mb\n")
    ...
    def sample(self) -> Dict:
        try:
            ...
            with open(self.csv_path, "a", encoding="utf-8") as f:
                f.write(
                    f"{timestamp},{elapsed:.2f},{memory_mb if memory_mb is not None else ''}\n"
                )
            return sample
        except Exception:
            return {"memory_mb": None, "memory_str": "Error"}
    ...
    def _get_memory_info_mb(self) -> Optional[float]:
        ...
        try:
            ...
            return None  # Catch all exceptions for robustness
    ...
    def get_report(self) -> Dict:
        ...
        valid_samples = [
            s["memory_mb"] for s in self.memory_samples if s["memory_mb"] is not None
        ]
        start_mem = valid_samples[0] if valid_samples else None
        end_mem = valid_samples[-1] if valid_samples else None
        max_mem = max(valid_samples) if valid_samples else None
        avg_mem = sum(valid_samples) / len(valid_samples) if valid_samples else None
        growth = (
            (end_mem - start_mem)
...
class CrawlerStressTest:
    ...
    def __init__(
        ...
        self.results_summary = {
            ...
            "rate_limiter_used": use_rate_limiter,
            "start_time": "",
            "end_time": "",
    ...
    async def run(self) -> Dict:
        ...
        memory_tracker = SimpleMemoryTracker(
            report_path=self.report_path, test_id=self.test_id
        )
        urls = [
            f"http://localhost:{self.server_port}/page_{i}.html"
        ...
        monitor = None
        rate_limiter = (
            RateLimiter(base_delay=(0.1, 0.3)) if self.use_rate_limiter else None
        )
        ...
        console.print(
            f"[bold cyan]Mode:[/bold cyan] {'Streaming' if self.stream_mode else 'Batch'}, [bold cyan]Monitor:[/bold cyan] {self.monitor_mode.name}, [bold cyan]Chunk Size:[/bold cyan] {self.chunk_size}"
        )
        ...
        if not self.stream_mode:
            ...
            console.print(
                "[bold] Batch | Progress | Start Mem | End Mem   | URLs/sec | Success/Fail | Time (s) | Status [/bold]"
            )
            console.print("─" * 90)

        monitor_task = asyncio.create_task(
            self._periodic_memory_sample(memory_tracker, 2.0)
        )
        ...
        try:
            async with AsyncWebCrawler(config=BrowserConfig(verbose=False)) as crawler:
                ...
                for chunk_idx, url_chunk in enumerate(url_chunks):
                    ...
                    try:
                        ...
                        results_gen_or_list: Union[
                            AsyncGenerator[CrawlResult, None], List[CrawlResult]
                        ...
                        console.print(
                            f"[bold red]Error processing chunk {chunk_idx+1}: {e}[/bold red]"
                        )
                        chunk_failed = len(
                            url_chunk
                        )  # Assume all failed in the chunk on error
                        total_urls_processed += len(
                            url_chunk
                        )  # Count them as processed (failed)

                    # --- Log batch results (only if not streaming) ---
                    if not self.stream_mode:
                        batch_time = time.time() - batch_start_time
                        urls_per_sec = (
                            len(url_chunk) / batch_time if batch_time > 0 else 0
                        )
                        ...
                        if chunk_failed == 0:
                            status_color, status = "green", "Success"
                        elif chunk_success == 0:
                            status_color, status = "red", "Failed"
                        else:
                            status_color, status = "yellow", "Partial"
            ...
            console.print(
                f"[bold red]An error occurred during the main crawl loop: {e}[/bold red]"
            )
        finally:
    ...
    async def _periodic_memory_sample(
        self, tracker: SimpleMemoryTracker, interval: float
    ):
...
async def run_full_test(args):
    ...
    if not args.use_existing_site and not args.skip_generation:
        if os.path.exists(args.site_path):
            console.print(
                f"[yellow]Removing existing site directory: {args.site_path}[/yellow]"
            )
            shutil.rmtree(args.site_path)
        site_generator = SiteGenerator(site_path=args.site_path, page_count=args.urls)
        site_generator.generate_site()
        site_generated = True
    elif args.use_existing_site:
        console.print(
            f"[cyan]Using existing site assumed to be running on port {args.port}[/cyan]"
        )
    elif args.skip_generation:
        console.print(
            f"[cyan]Skipping site generation, using existing directory: {args.site_path}[/cyan]"
        )
        if not os.path.exists(args.site_path) or not os.path.isdir(args.site_path):
            console.print(
                f"[bold red]Error: Site path '{args.site_path}' does not exist or is not a directory.[/bold red]"
            )
            return
    ...
    if not args.use_existing_site:
        ...
        try:
            ...
            console.print(
                "[bold red]Failed to start local server. Aborting test.[/bold red]"
            )
            if site_generated and not args.keep_site:
                console.print(
                    f"[yellow]Cleaning up generated site: {args.site_path}[/yellow]"
                )
                shutil.rmtree(args.site_path)
            return
    ...
    try:
        ...
        success_rate = (
            results["successful_urls"] / results["url_count"] * 100
            if results["url_count"] > 0
            else 0
        )
        urls_per_second = (
            results["urls_processed"] / results["total_time_seconds"]
        ...
        console.print(
            f"[bold cyan]Configuration:[/bold cyan] {results['url_count']} URLs, {results['max_sessions']} sessions, Chunk: {results['chunk_size']}, Stream: {results['stream_mode']}, Monitor: {results['monitor_mode']}"
        )
        console.print(
            f"[bold cyan]Results:[/bold cyan] {results['successful_urls']} successful, {results['failed_urls']} failed ({results['urls_processed']} processed, {success_rate:.1f}% success)"
        )
        console.print(
            f"[bold cyan]Performance:[/bold cyan] {results['total_time_seconds']:.2f} seconds total, {urls_per_second:.2f} URLs/second avg"
        )

        mem_report = results.get("memory", {})
        mem_info_str = "Memory tracking data unavailable."
        if mem_report and not mem_report.get("error"):
            start_mb = mem_report.get("start_memory_mb")
            end_mb = mem_report.get("end_memory_mb")
            max_mb = mem_report.get("max_memory_mb")
            growth_mb = mem_report.get("memory_growth_mb")
            mem_parts = []
            if start_mb is not None:
                mem_parts.append(f"Start: {start_mb:.1f} MB")
            if end_mb is not None:
                mem_parts.append(f"End: {end_mb:.1f} MB")
            if max_mb is not None:
                mem_parts.append(f"Max: {max_mb:.1f} MB")
            if growth_mb is not None:
                mem_parts.append(f"Growth: {growth_mb:.1f} MB")
            if mem_parts:
                mem_info_str = ", ".join(mem_parts)
            csv_path = mem_report.get("csv_path")
            if csv_path:
                console.print(f"[dim]Memory samples saved to: {csv_path}[/dim]")

        console.print(f"[bold cyan]Memory Usage:[/bold cyan] {mem_info_str}")
        console.print(
            f"[bold green]Results summary saved to {results['memory']['csv_path'].replace('memory_samples', 'test_summary').replace('.csv', '.json')}[/bold green]"
        )  # Infer summary path
        ...
        if results["urls_processed"] < results["url_count"]:
            console.print(
        ...
        if server_started and server and not args.keep_server_alive:
            server.stop()
        elif server_started and server and args.keep_server_alive:
            console.print(
                f"[bold cyan]Server is kept running on port {args.port}. Press Ctrl+C to stop it.[/bold cyan]"
            )
            try:
                await asyncio.Future()  # Keep running indefinitely
            except KeyboardInterrupt:
                console.print(
                    "\n[bold yellow]Stopping server due to user interrupt...[/bold yellow]"
                )
                server.stop()
        ...
        if site_generated and not args.keep_site:
            ...
            shutil.rmtree(args.site_path)
        elif args.clean_site and os.path.exists(args.site_path):
            console.print(
                f"[yellow]Cleaning up site directory as requested: {args.site_path}[/yellow]"
            )
            shutil.rmtree(args.site_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Crawl4AI SDK High Volume Stress Test using arun_many"
    )
    ...
    parser.add_argument(
        ...
        help=f"Number of URLs to test (default: {DEFAULT_URL_COUNT})",
    )
    parser.add_argument(
        "--max-sessions",
        type=int,
        default=DEFAULT_MAX_SESSIONS,
        help=f"Maximum concurrent crawling sessions (default: {DEFAULT_MAX_SESSIONS})",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=DEFAULT_CHUNK_SIZE,
        help=f"Number of URLs per batch for logging (default: {DEFAULT_CHUNK_SIZE})",
    )  # Added
    parser.add_argument(
        "--stream",
        action="store_true",
        default=DEFAULT_STREAM_MODE,
        help=f"Enable streaming mode (disables batch logging) (default: {DEFAULT_STREAM_MODE})",
    )
    parser.add_argument(
        "--monitor-mode",
        type=str,
        ...
        help=f"Display mode for the live monitor (default: {DEFAULT_MONITOR_MODE})",
    )
    parser.add_argument(
        "--use-rate-limiter",
        action="store_true",
        default=False,
        help="Enable a basic rate limiter (default: False)",
    )
    ...
    parser.add_argument(
        ...
        help=f"Path to generate/use the test site (default: {DEFAULT_SITE_PATH})",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=DEFAULT_PORT,
        help=f"Port for the local HTTP server (default: {DEFAULT_PORT})",
    )
    parser.add_argument(
        "--report-path",
        type=str,
        default=DEFAULT_REPORT_PATH,
        help=f"Path to save reports and logs (default: {DEFAULT_REPORT_PATH})",
    )
    ...
    parser.add_argument(
        "--skip-generation",
        action="store_true",
        help="Use existing test site folder without regenerating",
    )
    parser.add_argument(
        "--use-existing-site",
        action="store_true",
        help="Do not generate site or start local server; assume site exists on --port",
    )
    parser.add_argument(
        "--keep-server-alive",
        action="store_true",
        help="Keep the local HTTP server running after test",
    )
    parser.add_argument(
        "--keep-site",
        action="store_true",
        help="Keep the generated test site files after test",
    )
    parser.add_argument(
        "--clean-reports",
        action="store_true",
        help="Clean up report directory before running",
    )
    parser.add_argument(
        "--clean-site",
        action="store_true",
        help="Clean up site directory before running (if generating) or after",
    )
    ...
    # Display config
    console.print(
        "[bold underline]Crawl4AI SDK Stress Test Configuration[/bold underline]"
    )
    console.print(
        f"URLs: {args.urls}, Max Sessions: {args.max_sessions}, Chunk Size: {args.chunk_size}"
    )  # Added chunk size
    console.print(
        f"Mode: {'Streaming' if args.stream else 'Batch'}, Monitor: {args.monitor_mode}, Rate Limit: {args.use_rate_limiter}"
    )
    console.print(
        f"Site Path: {args.site_path}, Port: {args.port}, Report Path: {args.report_path}"
    )
    console.print("-" * 40)
    # (Rest of config display and cleanup logic is the same)
    if args.use_existing_site:
        console.print("[cyan]Mode: Using existing external site/server[/cyan]")
    elif args.skip_generation:
        console.print(
            "[cyan]Mode: Using existing site files, starting local server[/cyan]"
        )
    else:
        console.print("[cyan]Mode: Generating site files, starting local server[/cyan]")
    if args.keep_server_alive:
        console.print("[cyan]Option: Keep server alive after test[/cyan]")
    if args.keep_site:
        console.print("[cyan]Option: Keep site files after test[/cyan]")
    if args.clean_reports:
        console.print("[cyan]Option: Clean reports before test[/cyan]")
    if args.clean_site:
        console.print("[cyan]Option: Clean site directory[/cyan]")
    console.print("-" * 40)

    if args.clean_reports:
        if os.path.exists(args.report_path):
            console.print(
                f"[yellow]Cleaning up reports directory: {args.report_path}[/yellow]"
            )
