...
class UserHookManager:
    ...
    def validate_hook_structure(
        self, hook_code: str, hook_point: str
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if not func_def:
                return (
                    False,
                    "Hook must contain a function definition (def or async def)",
                )
            ...
            if not isinstance(func_def, ast.AsyncFunctionDef):
                return (
                    False,
                    "Hook function must be async (use 'async def' instead of 'def')",
                )
            ...
            if missing_params and not has_kwargs:
                return (
                    False,
                    f"Hook function '{func_name}' must accept parameters: {', '.join(expected_params)} (missing: {', '.join(missing_params)})",
                )
            ...
            if not has_return:
                ...
                logger.warning(
                    f"Hook function '{func_name}' should return the {expected_params[0]} object"
                )
    ...
    async def execute_hook_safely(
        self, hook_func: Callable, hook_point: str, *args, **kwargs
        ...
        try:
            ...
            result = await asyncio.wait_for(
                hook_func(*args, **kwargs), timeout=self.timeout
            )
        ...
        async def wrapped_hook(*args, **kwargs):
            ...
            try:
                ...
                result, error = await self.hook_manager.execute_hook_safely(
                    user_hook, hook_point, *args, **kwargs
                )

                if error:
                    # Hook failed but we continue with original object
                    logger.warning(
                        f"User hook failed at {hook_point}: {error['error']}"
                    )
                    return return_obj
                ...
                if result is None:
                    logger.debug(
                        f"Hook at {hook_point} returned None, using original object"
                    )
                    return return_obj
...
async def process_user_hooks(
    hooks_input: Dict[str, str], timeout: int = 30
            ...
            validation_errors.append(
                {
                    ...
                    "code_preview": (
                        hook_code[:100] + "..." if len(hook_code) > 100 else hook_code
                    ),
                }
            )
            continue
...
async def process_user_hooks_with_manager(
    hooks_input: Dict[str, str], hook_manager: UserHookManager
...
async def attach_user_hooks_to_crawler(
    ...
    for hook_point, wrapped_hook in compiled_hooks.items():
        try:
            ...
            validation_errors.append(
                {"hook_point": hook_point, "error": f"Failed to attach hook: {str(e)}"}
            )

    status = (
        "success"
        if not validation_errors
        else ("partial" if attached_hooks else "failed")
    )
