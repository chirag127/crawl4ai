        ...
        for table in tables:
            ...
            if self.is_data_table(table, table_score_threshold=score_threshold):
                try:
                    ...
                    if (
                        self.min_rows > 0
                        and len(table_data.get("rows", [])) < self.min_rows
                    ):
                        continue
                    if self.min_cols > 0:
                        col_count = len(table_data.get("headers", [])) or (
                            max(len(row) for row in table_data.get("rows", []))
                    ...
                    self._log(
                        "error",
                        f"Error extracting table data: {str(e)}",
                        "TABLE_EXTRACT",
                    )
                    continue
...
**CRITICAL**: Your response must be a valid JSON object that conforms to this schema. The entire purpose of using an LLM for this task is to handle complex HTML tables that standard parsers cannot process correctly. Your value lies in intelligently interpreting complex structures and returning complete, clean, tabulated data in valid JSON format."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.min_rows_per_chunk = max(
            5, min_rows_per_chunk
        )  # At least 5 rows per chunk
    ...
    def extract_tables(self, element: etree.Element, **kwargs) -> List[Dict[str, Any]]:
        ...
        if css_selector:
            ...
            if not selected_elements:
                self._log(
                    "warning", f"No elements found for CSS selector: {css_selector}"
                )
                return []
            html_content = "".join(
                etree.tostring(elem, encoding="unicode") for elem in selected_elements
            )
        else:
        ...
        if self.verbose:
            self._log(
                "info", f"Found table tags in HTML, content length: {len(html_content)}"
            )
        ...
        if self.enable_chunking and self._needs_chunking(html_content):
            if self.verbose:
                self._log(
                    "info", "Content exceeds token threshold, using chunked extraction"
                )
...
Return only a JSON array of extracted tables following the specified format."""
        ...
        for attempt in range(1, self.max_tries + 1):
            try:
                if self.verbose and attempt > 1:
                    self._log(
                        "info",
                        f"Retry attempt {attempt}/{self.max_tries} for table extraction",
                    )
                ...
                if response and response.choices:
                    ...
                    if self.verbose:
                        ...
                        if isinstance(content, str):
                            self._log(
                                "debug", f"LLM response preview: {content[:200]}..."
                            )
                    ...
                    while (
                        isinstance(tables_data, list)
                        and len(tables_data) == 1
                        and isinstance(tables_data[0], list)
                    ):
                    ...
                    if self.verbose:
                        self._log(
                            "debug",
                            f"Parsed {len(tables_data)} table(s) from LLM response",
                        )
                    ...
                    if validated_tables:
                        if self.verbose:
                            self._log(
                                "info",
                                f"Successfully extracted {len(validated_tables)} tables using LLM on attempt {attempt}",
                            )
                    ...
                    if attempt < self.max_tries:
                        if self.verbose:
                            self._log(
                                "warning",
                                f"No valid tables extracted on attempt {attempt}, retrying...",
                            )
                        continue
                    else:
                        if self.verbose:
                            self._log(
                                "warning",
                                f"No valid tables extracted after {self.max_tries} attempts",
                            )
                        return []
                ...
                if self.verbose:
                    self._log(
                        "error", f"JSON parsing error on attempt {attempt}: {str(e)}"
                    )
                if attempt < self.max_tries:
                    continue
                else:
                ...
                if self.verbose:
                    self._log(
                        "error",
                        f"Error in LLM table extraction on attempt {attempt}: {str(e)}",
                    )
                    if attempt == self.max_tries:
                        import traceback
    ...
    def _needs_chunking(self, html_content: str) -> bool:
        ...
        if self.verbose and needs_chunk:
            self._log(
                "info",
                f"Table needs chunking: {token_count} tokens > {self.chunk_token_threshold} threshold",
            )
    ...
    ) -> Tuple[List[etree.Element], List[etree.Element], List[etree.Element], bool]:
        """
    ...
    def _create_smart_chunks(self, html_content: str) -> Tuple[List[str], bool]:
        ...
        if self.verbose:
            self._log(
                "info",
                f"Creating smart chunks from {len(html_content)} characters of HTML",
            )

        header_rows, body_rows, footer_rows, has_headers = (
            self._extract_table_structure(html_content)
        )

        if self.verbose:
            self._log(
                "info",
                f"Table structure: {len(header_rows)} header rows, {len(body_rows)} body rows, {len(footer_rows)} footer rows",
            )
        ...
        for row in body_rows:
            ...
            if current_chunk_rows and (
                current_token_count + row_tokens > self.chunk_token_threshold
            ):
                # Create chunk with current rows
                chunk_html = self._create_chunk_html(
                    header_html, current_chunk_rows, None
                )
        ...
        if current_chunk_rows:
            ...
            chunk_html = self._create_chunk_html(
                header_html, current_chunk_rows, footer_html
            )
            chunks.append(chunk_html)
    ...
    def _create_chunk_html(
        self, header_html: str, body_rows: List[str], footer_html: Optional[str]
    ) -> str:
        """
...
Return only a JSON array of extracted tables following the specified format."""
        ...
        for attempt in range(1, self.max_tries + 1):
            try:
                if self.verbose and attempt > 1:
                    self._log(
                        "info",
                        f"Retry attempt {attempt}/{self.max_tries} for chunk {chunk_index + 1}",
                    )
                ...
                if self.verbose:
                    self._log(
                        "error", f"Error processing chunk {chunk_index + 1}: {str(e)}"
                    )
    ...
    def _merge_chunk_results(
        ...
        if self.verbose:
            self._log(
                "info",
                f"Merged {len(valid_chunks)} chunks into table with {len(all_rows)} rows",
            )
    ...
    def _extract_with_chunking(self, html_content: str) -> List[Dict[str, Any]]:
        ...
        if self.verbose:
            self._log(
                "info",
                f"Starting chunked extraction for content with {len(html_content)} characters",
            )
        ...
        if len(chunks) == 1:
            ...
            if self.verbose:
                self._log(
                    "info", "Processing as single chunk (no parallelization needed)"
                )
        ...
        if self.verbose:
            self._log(
                "info",
                f"Processing {len(chunks)} chunks in parallel (max workers: {self.max_parallel_chunks})",
            )
        ...
        with ThreadPoolExecutor(max_workers=self.max_parallel_chunks) as executor:
            ...
            futures = {
                executor.submit(
                    self._process_chunk, chunk, i, len(chunks), has_headers
                ): i
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if self.verbose:
                        self._log(
                            "info",
                            f"Chunk {chunk_index + 1}/{len(chunks)} completed successfully",
                        )
                    ...
                    if self.verbose:
                        self._log(
                            "error",
                            f"Chunk {chunk_index + 1}/{len(chunks)} processing failed: {str(e)}",
                        )
                    chunk_results.append(
                        {"chunk_index": chunk_index, "table": None, "error": str(e)}
                    )
    ...
    def _css_to_xpath_select(
        self, element: etree.Element, css_selector: str
    ) -> List[etree.Element]:
        """
        ...
        try:
            ...
            self._log(
                "warning",
                f"XPath conversion failed for selector '{css_selector}': {str(e)}",
            )
            return []
