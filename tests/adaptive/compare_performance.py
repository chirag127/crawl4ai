...
def read_baseline():
    ...
    for i, line in enumerate(lines):
        if "Total Time:" in line:
            ...
            "validate_coverage:" in line
            and i + 1 < len(lines)
            and "Avg Time:" in lines[i + 2]
        ):
            metrics["validate_coverage_ms"] = float(
                lines[i + 2].split(":")[1].strip().split()[0]
            )
        elif (
            "select_links:" in line
            and i + 1 < len(lines)
            and "Avg Time:" in lines[i + 2]
        ):
            metrics["select_links_ms"] = float(
                lines[i + 2].split(":")[1].strip().split()[0]
            )
        elif (
            "calculate_confidence:" in line
            and i + 1 < len(lines)
            and "Avg Time:" in lines[i + 2]
        ):
            metrics["calculate_confidence_ms"] = float(
                lines[i + 2].split(":")[1].strip().split()[0]
            )
...
def print_comparison(before_metrics, after_metrics):
    ...
    # Total time
    time_improvement = (
        (before_metrics["total_time"] - after_metrics["total_time"])
        / before_metrics["total_time"]
        * 100
    )
    print("\n📊 Total Time:")
    ...
    print(
        f"   Improvement: {time_improvement:.1f}% faster ✅"
        if time_improvement > 0
        else f"   Slower: {-time_improvement:.1f}% ❌"
    )

    # Memory
    mem_improvement = (
        (before_metrics["memory_mb"] - after_metrics["memory_mb"])
        / before_metrics["memory_mb"]
        * 100
    )
    print("\n💾 Memory Usage:")
    ...
    print(
        f"   Improvement: {mem_improvement:.1f}% less memory ✅"
        if mem_improvement > 0
        else f"   More memory: {-mem_improvement:.1f}% ❌"
    )
    ...
    # Validate coverage
    if (
        "validate_coverage_ms" in before_metrics
        and "validate_coverage_ms" in after_metrics
    ):
        val_improvement = (
            (
                before_metrics["validate_coverage_ms"]
                - after_metrics["validate_coverage_ms"]
            )
            / before_metrics["validate_coverage_ms"]
            * 100
        )
        ...
        print(
            f"     Improvement: {val_improvement:.1f}% faster ✅"
            if val_improvement > 0
            else f"     Slower: {-val_improvement:.1f}% ❌"
        )

    # Select links
    if "select_links_ms" in before_metrics and "select_links_ms" in after_metrics:
        sel_improvement = (
            (before_metrics["select_links_ms"] - after_metrics["select_links_ms"])
            / before_metrics["select_links_ms"]
            * 100
        )
        print("\n   select_links:")
        ...
        print(
            f"     Improvement: {sel_improvement:.1f}% faster ✅"
            if sel_improvement > 0
            else f"     Slower: {-sel_improvement:.1f}% ❌"
        )

    # Calculate confidence
    if (
        "calculate_confidence_ms" in before_metrics
        and "calculate_confidence_ms" in after_metrics
    ):
        calc_improvement = (
            (
                before_metrics["calculate_confidence_ms"]
                - after_metrics["calculate_confidence_ms"]
            )
            / before_metrics["calculate_confidence_ms"]
            * 100
        )
        ...
        print(
            f"     Improvement: {calc_improvement:.1f}% faster ✅"
            if calc_improvement > 0
            else f"     Slower: {-calc_improvement:.1f}% ❌"
        )
