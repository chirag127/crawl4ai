...
async def check_server_health(
    client: httpx.AsyncClient, health_endpoint: str = "/health"
):
    """Check if the API server is healthy."""
    console.print(
        f"[bold cyan]Checking API server health at {client.base_url}{health_endpoint}...[/]",
        end="",
    )
    try:
...
class ApiStressTest:
    ...
    async def run(self) -> Dict:
        ...
        urls_to_process = [
            f"https://httpbin.org/anything/{uuid.uuid4()}"
            for _ in range(self.url_count)
        ]
        url_chunks = [
            urls_to_process[i : i + self.chunk_size]
            for i in range(0, len(urls_to_process), self.chunk_size)
        ]
        ...
        console.print(
            f"[bold cyan]Target API:[/bold cyan] {self.api_base_url}, [bold cyan]Mode:[/bold cyan] {'Streaming' if self.stream_mode else 'Batch'}, [bold cyan]URLs per Request:[/bold cyan] {self.chunk_size}"
        )
        ...
        console.print(
            "[bold] Batch | Progress | SrvMem Peak / Δ|Max (MB) | Reqs/sec | S/F URLs | Time (s) | Status  [/bold]"
        )
        ...
        try:
            ...
            total_successful_requests = sum(
                1 for r in api_results if r["request_success"]
            )
            ...
            valid_samples = [
                r for r in api_results if r.get("server_delta_or_max_mb") is not None
            ]  # Filter results with valid mem data
            self.results_summary["server_memory_metrics"][
                "samples"
            ] = valid_samples  # Store raw samples with both peak and delta/max

            if valid_samples:
                delta_or_max_values = [
                    r["server_delta_or_max_mb"] for r in valid_samples
                ]
                if self.stream_mode:
                    # Stream mode: delta_or_max holds max snapshot
                    self.results_summary["server_memory_metrics"][
                        "stream_mode_avg_max_snapshot_mb"
                    ] = sum(delta_or_max_values) / len(delta_or_max_values)
                    self.results_summary["server_memory_metrics"][
                        "stream_mode_max_max_snapshot_mb"
                    ] = max(delta_or_max_values)
                else:  # Batch mode
                    # delta_or_max holds delta
                    self.results_summary["server_memory_metrics"][
                        "batch_mode_avg_delta_mb"
                    ] = sum(delta_or_max_values) / len(delta_or_max_values)
                    self.results_summary["server_memory_metrics"][
                    ...
                    peak_values = [
                        r["server_peak_memory_mb"]
                        for r in valid_samples
                        if r.get("server_peak_memory_mb") is not None
                    ]
                    if peak_values:
                        self.results_summary["server_memory_metrics"][
                            "batch_mode_avg_peak_mb"
                        ] = sum(peak_values) / len(peak_values)
                        self.results_summary["server_memory_metrics"][
                            "batch_mode_max_peak_mb"
                        ] = max(peak_values)
            ...
            console.print(
                f"[bold red]An error occurred during task execution: {e}[/bold red]"
            )
            import traceback
    ...
    async def _make_api_request(
        ...
        async with semaphore:
            try:
                ...
                if self.stream_mode:
                    max_server_mem_snapshot = (
                        0.0  # Track max memory seen in this stream
                    )
                                ...
                                try:
                                    ...
                                    if data.get("status") == "completed":
                                        ...
                                        if mem_snapshot is not None:
                                            max_server_mem_snapshot = max(
                                                max_server_mem_snapshot,
                                                float(mem_snapshot),
                                            )
                                except json.JSONDecodeError:
                                    console.print(
                                        f"[Batch {batch_idx}] [red]Stream decode error for line:[/red] {line}"
                                    )
                    ...
                    if data.get("success") and "results" in data:
                        ...
                        if len(results_list) != len(chunk):
                            console.print(
                                f"[Batch {batch_idx}] [yellow]Warning: Result count ({len(results_list)}) doesn't match URL count ({len(chunk)})[/yellow]"
                            )
                            failed_urls = len(chunk) - success_urls
                    else:
                        request_success = False
                        ...
                        if isinstance(detail, str):
                            ...
                            detail_json = {}
                        server_peak_mem_mb = detail_json.get(
                            "server_peak_memory_mb", None
                        )
                        server_memory_metric = detail_json.get(
                            "server_memory_delta_mb", None
                        )
                        console.print(
                            f"[Batch {batch_idx}] [red]API request failed:[/red] {detail_json.get('error', 'No details')}"
                        )
                ...
                failed_urls = len(chunk)
                console.print(
                    f"[Batch {batch_idx}] [bold red]HTTP Error {e.response.status_code}:[/] {e.request.url}"
                )
                try:
                    ...
                    if isinstance(
                        detail_content, str
                    ):  # Handle if detail is stringified JSON
                        try:
                            detail_content = json.loads(detail_content)
                        except:
                            detail_content = {}
                    server_memory_metric = detail_content.get(
                        "server_memory_delta_mb", None
                    )
                    server_peak_mem_mb = detail_content.get(
                        "server_peak_memory_mb", None
                    )
                ...
                failed_urls = len(chunk)
                console.print(
                    f"[Batch {batch_idx}] [bold red]Request Error:[/bold] {e.request.url} - {e}"
                )
            except Exception as e:
                request_success = False
                failed_urls = len(chunk)
                console.print(
                    f"[Batch {batch_idx}] [bold red]Unexpected Error:[/bold] {e}"
                )
                import traceback
                ...
                if request_success and failed_urls == 0:
                    status_color, status = "green", "Success"
                elif request_success and success_urls > 0:
                ...
                reqs_per_sec = (
                    1.0 / api_call_time if api_call_time > 0 else float("inf")
                )
                ...
                if self.stream_mode:
                    ...
                    if server_memory_metric is not None:
                        ...
                        delta_or_max_value = (
                            server_memory_metric  # Store for aggregation
                        )
                else:  # Batch mode - expect peak and delta
                    # We need to get peak and delta from the API response
                    peak_mem_value = locals().get(
                        "server_peak_mem_mb", None
                    )  # Get from response data if available
...
async def run_full_test(args):
    ...
    success_rate_reqs = (
        results["successful_requests"] / results["total_api_calls"] * 100
    ...
    urls_per_second = (
        results["total_urls_processed"] / results["total_time_seconds"]
        if results["total_time_seconds"] > 0
        else 0
    )
    reqs_per_second = (
        results["total_api_calls"] / results["total_time_seconds"]
        if results["total_time_seconds"] > 0
        else 0
    )
    ...
    console.print(
        f"[bold cyan]Configuration:[/bold cyan] {results['url_count']} URLs, {results['max_concurrent_requests']} concurrent client requests, URLs/Req: {results['chunk_size']}, Stream: {results['stream_mode']}"
    )
    console.print(
        f"[bold cyan]API Requests:[/bold cyan] {results['successful_requests']} successful, {results['failed_requests']} failed ({results['total_api_calls']} total, {success_rate_reqs:.1f}% success)"
    )
    console.print(
        f"[bold cyan]URL Processing:[/bold cyan] {results['successful_urls']} successful, {results['failed_urls']} failed ({results['total_urls_processed']} processed, {success_rate_urls:.1f}% success)"
    )
    console.print(
        f"[bold cyan]Performance:[/bold cyan] {results['total_time_seconds']:.2f}s total | Avg Reqs/sec: {reqs_per_second:.2f} | Avg URLs/sec: {urls_per_second:.2f}"
    )
    ...
    if mem_samples:
        ...
        if results["stream_mode"]:
            ...
            console.print(
                f"[bold cyan]Server Memory (Stream):[/bold cyan] Avg Max Snapshot: {avg_str} MB | Max Max Snapshot: {max_str} MB (across {num_samples} requests)"
            )
        else:  # Batch mode
            ...
            console.print(
                f"[bold cyan]Server Memory (Batch):[/bold cyan] Avg Peak: {avg_peak_str} MB | Max Peak: {max_peak_str} MB | Avg Delta: {avg_delta_str} MB | Max Delta: {max_delta_str} MB (across {num_samples} requests)"
            )
    else:
        console.print(
            "[bold cyan]Server Memory:[/bold cyan] No memory data reported by server."
        )

    # No client memory report
    summary_path = (
        pathlib.Path(args.report_path) / f"api_test_summary_{results['test_id']}.json"
    )
    ...
    if results["failed_urls"] > 0:
        console.print(
            f"[bold yellow]Warning: {results['failed_urls']} URLs failed to process ({100-success_rate_urls:.1f}% URL failure rate)[/bold yellow]"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Base URL of the Crawl4AI API server (default: {DEFAULT_API_URL})",
    )
    parser.add_argument(
        "--urls",
        type=int,
        default=DEFAULT_URL_COUNT,
        help=f"Total number of unique URLs to process via API calls (default: {DEFAULT_URL_COUNT})",
    )
    parser.add_argument(
        "--max-concurrent-requests",
        type=int,
        default=DEFAULT_MAX_CONCURRENT_REQUESTS,
        help=f"Maximum concurrent API requests from this client (default: {DEFAULT_MAX_CONCURRENT_REQUESTS})",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=DEFAULT_CHUNK_SIZE,
        help=f"Number of URLs per API request payload (default: {DEFAULT_CHUNK_SIZE})",
    )
    parser.add_argument(
        "--stream",
        action="store_true",
        default=DEFAULT_STREAM_MODE,
        help=f"Use the /crawl/stream endpoint instead of /crawl (default: {DEFAULT_STREAM_MODE})",
    )
    parser.add_argument(
        "--report-path",
        type=str,
    ...
    console.print(
        "[bold underline]Crawl4AI API Stress Test Configuration[/bold underline]"
    )
    console.print(f"API URL: {args.api_url}")
    console.print(
        f"Total URLs: {args.urls}, Concurrent Client Requests: {args.max_concurrent_requests}, URLs per Request: {args.chunk_size}"
    )
