...
from crawl4ai import (AsyncLoggerBase, AsyncWebCrawler, BrowserConfig,
...
async def main():
    ...
    crawler = AsyncWebCrawler(
        ...
        logger=AsyncFileLogger("/Users/unclecode/devs/crawl4ai/.private/tmp/crawl.log"),
    )
    await crawler.start()
    ...
    try:
        ...
        result = await crawler.arun(url="https://kidocode.com/", config=crawl_config)
        if result.success:
            print(
                "First crawl - Raw Markdown Length:", len(result.markdown.raw_markdown)
            )
