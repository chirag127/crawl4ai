...
class TestAsyncUrlSeederBM25:
    ...
    async def test_jsonld_extraction_scoring(self, seeder):
        ...
        if jsonld_results:
            ...
            for r in jsonld_results[:3]:
                ...
                print(
                    f"    JSON-LD types: {[item.get('@type', 'Unknown') for item in jsonld_data if isinstance(item, dict)]}"
                )
    ...
    async def test_multi_word_complex_queries(self, seeder):
        ...
        for query in complex_queries:
            ...
            if results:
                print(f"\nQuery: '{query}'")
                print(
                    f"Top match: {results[0]['relevance_score']:.3f} - {results[0]['url']}"
                )
    ...
    async def test_generate_scoring_report(self, seeder):
        ...
        for category, data in report["results"].items():
            ...
            print(
                f"  Score range: {data['score_distribution']['min']:.3f} - {data['score_distribution']['max']:.3f}"
            )
