...
check(
    ...
    is_blocked(
        403,
        "<html><head><title>Pardon Our Interruption</title></head><body><p>Please verify you are human</p></body></html>",
    ),
    True,
    "Pardon",
)
...
check(
    "Akamai 403 short Access Denied",
    is_blocked(403, "<html><body><h1>Access Denied</h1></body></html>"),
    True,
)  # Detected via near-empty 403 or Access Denied pattern
...
check(
    ...
    True,
)  # Detected via near-empty 403 or Cloudflare pattern

check(
    "Cloudflare Checking your browser (short 503)",
    is_blocked(
        503,
...
check(
    ...
    is_blocked(
        403,
        """<html><head><title>Access to This Page Has Been Blocked</title></head>
...
check(
    ...
    is_blocked(
        403,
        '<html><body><script src="https://captcha.px-cdn.net/PX12345/captcha.js"></script></body></html>',
    ),
    True,
    "PerimeterX captcha",
)
...
# --- Imperva/Incapsula ---
check(
    "Imperva Incapsula Resource",
    is_blocked(
        403,
        '<html><body><iframe src="/_Incapsula_Resource?incident_id=123&sess_id=abc"></iframe></body></html>',
    ),
    True,
    "Imperva",
)
...
check(
    ...
    is_blocked(
        200,
        "<html><body>Request unsuccessful. Incapsula incident ID: 12345-67890</body></html>",
    ),
    True,
    "Incapsula incident",
)
...
check(
    ...
    is_blocked(
        403,
        "<html><body><h1>Sucuri WebSite Firewall - Access Denied</h1></body></html>",
    ),
    True,
    "Sucuri",
)
...
check(
    "Reddit blocked by network security (small)",
    is_blocked(
        403, "<html><body>You've been blocked by network security.</body></html>"
    ),
    True,
...
check(
    "Reddit blocked by network security (190KB SPA shell)",
    is_blocked(
        403,
        "<html><body><style>"
        + "x" * 180000
...
check(
    "Network security block on HTTP 200 (buried in large page)",
    is_blocked(
        200,
        "<html><body><style>"
        + "a:b;" * 30000
...
check("HTTP 429 empty body", is_blocked(429, ""), True, "429")

# --- Empty 200 ---
check("HTTP 200 empty page", is_blocked(200, ""), True, "empty")

check("HTTP 200 whitespace only", is_blocked(200, "   \n\n   "), True, "empty")

# --- 403 near-empty ---
check("HTTP 403 near-empty (10 bytes)", is_blocked(403, "<html></html>"), True, "403")
...
# --- Normal pages ---
check(
    "Normal 200 page (example.com size)",
    is_blocked(
        200,
...
check(
    "Normal 200 large page",
    is_blocked(
        200, "<html><body>" + "<p>Some content here.</p>\n" * 5000 + "</body></html>"
    ),
    False,
)
...
check(
    "Article about bot detection (large page)",
    is_blocked(
        200,
        "<html><head><title>How to Detect Bots</title></head><body>"
...
check(
    "DataDome marketing page (large)",
    is_blocked(
        200,
        "<html><body><h1>DataDome Bot Protection</h1>"
...
check(
    "Login page with reCAPTCHA (large page)",
    is_blocked(
        200,
...
check(
    "Signup page with hCaptcha (large page)",
    is_blocked(
        200,
        "<html><body>"
...
check(
    "Apache directory listing denied (403, large-ish)",
    is_blocked(
        403,
...
check(
    "API 403 auth required (JSON)",
    is_blocked(
        403, '{"error": "Forbidden", "message": "Invalid API key", "code": 403}'
    ),
    False,
)
...
check(
    "Cloudflare-served normal page with footer",
    is_blocked(
        200,
        "<html><body>"
...
check(
    "Small valid 200 page (with content element)",
    is_blocked(
        200,
        "<html><head><title>OK</title></head><body><p>Your request was processed successfully. Everything is fine.</p></body></html>",
    ),
    False,
)

check(
    "Small JSON 200 response",
    is_blocked(
        200,
        '{"status": "ok", "data": {"id": 123, "name": "test"}, "timestamp": "2024-01-01T00:00:00Z"}',
    ),
    False,
)
...
check(
    ...
    is_blocked(
        200,
        '<html><head><meta http-equiv="refresh" content="0;url=/dashboard"></head><body><p>Redirecting to your dashboard. Please wait while we prepare your personalized experience.</p></body></html>',
    ),
    False,
)
...
check(
    "503 maintenance page (treated as blocked)",
    is_blocked(
        503,
...
check(
    "Short thank you page (200, 120 bytes)",
    is_blocked(
        200,
        "<html><body><h1>Thank You!</h1><p>Your order has been placed. Confirmation email sent.</p></body></html>",
    ),
    False,
)
...
check("None status code + empty html", is_blocked(None, ""), True, "no <body>")

check(
    "None status code + block content",
    is_blocked(
        None, "<html><body>Reference #18.2d351ab8.1557333295.a4e16ab</body></html>"
    ),
    True,
    "Akamai",
)
...
check(
    "200 + tier1 pattern (Imperva deceptive 200)",
    is_blocked(
        200,
        "<html><body>Request unsuccessful. Incapsula incident ID: 555-999</body></html>",
    ),
    True,
    "Incapsula",
)
...
check(
    "403 + 4999 bytes (just under threshold)",
...
check(
    "403 + 5001 bytes (over old threshold, now blocked)",
...
check(
    "403 + 9999 bytes with generic block text",
...
check(
    "403 + 10001 bytes with generic block text (now detected regardless of size)",
...
check(
    "200 + whitespace-padded but 89 bytes content (above threshold for meaningful)",
...
check(
    "200 + exactly 100 bytes stripped (at threshold, no body = structural fail)",
