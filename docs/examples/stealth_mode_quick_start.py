...
async def example_1_basic_stealth():
    ...
    browser_config = BrowserConfig(
        enable_stealth=True, headless=True  # This is the key parameter
    )
...
async def example_2_stealth_with_screenshot():
    ...
    browser_config = BrowserConfig(
        enable_stealth=True, headless=False  # Set to False to see the browser
    )
...
async def example_3_stealth_for_protected_sites():
    ...
    browser_config = BrowserConfig(
        enable_stealth=True, headless=True, viewport_width=1920, viewport_height=1080
    )
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        result = await crawler.arun(
            url="https://www.g2.com/products/slack/reviews", config=config
        )
...
async def example_4_stealth_with_sessions():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        result1 = await crawler.arun(url="https://news.ycombinator.com", config=config)
        ...
        result2 = await crawler.arun(
            url="https://news.ycombinator.com/best", config=config
        )
...
async def example_5_stealth_comparison():
    ...
    print("\nWithout stealth:")
    regular_config = BrowserConfig(enable_stealth=False, headless=True)
    ...
    print("\nWith stealth:")
    stealth_config = BrowserConfig(enable_stealth=True, headless=True)
