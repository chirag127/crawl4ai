...
def test_llm_job_payload_model():
    ...
    try:
        ...
        minimal_payload = {"url": "https://example.com", "q": "Extract content"}
...
def test_handle_llm_request_signature():
    ...
    try:
        ...
        if "webhook_config" in params:
            ...
            if (
                webhook_param.default is None
                or webhook_param.default == inspect.Parameter.empty
            ):
                print(
                    f"✅ webhook_config is optional (default: {webhook_param.default})"
                )
            else:
...
def test_webhook_integration_in_api():
    ...
    try:
        ...
        if "webhook_service = WebhookDeliveryService(config)" in api_content:
            ...
            print(
                "❌ Missing WebhookDeliveryService initialization in process_llm_extraction"
            )
            return False
        ...
        notification_count = api_content.count(
        ...
        llm_notification_count = llm_func_content.count(
            "await webhook_service.notify_job_completion"
        )

        print(
            f"✅ Found {llm_notification_count} webhook notification calls in process_llm_extraction"
        )
        ...
        if llm_notification_count >= 3:
            ...
            print(
                f"⚠️  Expected at least 3 notification calls, found {llm_notification_count}"
            )
...
def test_create_new_task_integration():
    ...
    try:
        ...
        if (
            "webhook_config" in create_task_func
            and "background_tasks.add_task" in create_task_func
        ):
...
def test_pattern_consistency():
    ...
    try:
        ...
        crawl_has_service = (
            "webhook_service = WebhookDeliveryService(config)" in crawl_job_func
        )
        llm_has_service = (
            "webhook_service = WebhookDeliveryService(config)" in llm_extract_func
        )
        ...
        if crawl_has_service and llm_has_service:
            ...
            print(
                f"❌ Service initialization mismatch (crawl: {crawl_has_service}, llm: {llm_has_service})"
            )
            return False

        # Both should call notify_job_completion on success
        crawl_notifies_success = (
            'status="completed"' in crawl_job_func
            and "notify_job_completion" in crawl_job_func
        )
        llm_notifies_success = (
            'status="completed"' in llm_extract_func
            and "notify_job_completion" in llm_extract_func
        )
        ...
        if crawl_notifies_success and llm_notifies_success:
            ...
            print(
                f"❌ Success notification mismatch (crawl: {crawl_notifies_success}, llm: {llm_notifies_success})"
            )
            return False

        # Both should call notify_job_completion on failure
        crawl_notifies_failure = (
            'status="failed"' in crawl_job_func and "error=" in crawl_job_func
        )
        llm_notifies_failure = (
            'status="failed"' in llm_extract_func and "error=" in llm_extract_func
        )
        ...
        if crawl_notifies_failure and llm_notifies_failure:
            ...
            print(
                f"❌ Failure notification mismatch (crawl: {crawl_notifies_failure}, llm: {llm_notifies_failure})"
            )
            return False
...
def main():
    ...
    results.append(
        ("handle_llm_request Signature", test_handle_llm_request_signature())
    )
    results.append(
        ("process_llm_extraction Signature", test_process_llm_extraction_signature())
    )
