...
from crawl4ai import (AsyncWebCrawler, BrowserConfig, CacheMode,
                      ...
                      HTTPCrawlerConfig, JsonCssExtractionStrategy, LLMConfig,
                      MemoryAdaptiveDispatcher, ProxyConfig,
                      RoundRobinProxyStrategy)
from crawl4ai.async_crawler_strategy import AsyncHTTPCrawlerStrategy
from crawl4ai.content_filter_strategy import LLMContentFilter
from crawl4ai.content_scraping_strategy import LXMLWebScrapingStrategy
from crawl4ai.deep_crawling import (BestFirstCrawlingStrategy,
                                    ContentTypeFilter, DomainFilter,
...
async def deep_crawl():
    ...
    print("\n===== DEEP CRAWLING =====")
    print(
        "This example shows how to implement deep crawling with filters, scorers, and robots.txt compliance."
    )
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        async for result in await crawler.arun(
            url="https://docs.crawl4ai.com", config=deep_crawl_config
        ):
...
async def memory_adaptive_dispatcher():
    ...
    print(
        "This example shows how to use the memory-adaptive dispatcher for resource management."
    )
    ...
    async def batch_mode():
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            print(
                f"  ✅ Received all {len(results)} results after {time.perf_counter() - start_time:.2f} seconds"
            )
            for result in results:
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            async for result in await crawler.arun_many(
                ...
                if count == 1:
                    ...
                    print(
                        f"  ✅ First result after {first_result_time:.2f} seconds: {result.url}"
                    )
                else:
                    print(
                        f"  → Result #{count} after {current_time:.2f} seconds: {result.url}"
                    )
...
async def proxy_rotation():
    ...
    print("\n===== PROXY ROTATION =====")
    print(
        "This example shows how to implement proxy rotation for distributed crawling."
    )
    ...
    run_config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS, proxy_rotation_strategy=proxy_strategy
    )
            ...
            for result in results:
                if result.success:
                    ...
                    if current_proxy and ip_match:
                        ...
                        print(
                            f"Proxy {current_proxy.server} -> Response IP: {ip_match.group(0)}"
                        )
...
async def llm_content_filter():
    ...
    markdown_generator = DefaultMarkdownGenerator(
        content_filter=LLMContentFilter(
            llm_config=llm_config, instruction="Extract key concepts and summaries"
        )
    )
...
async def llm_schema_generation():
    ...
    print(
        "This example shows how to use LLM to automatically generate extraction schemas."
    )
    print("Note: This example requires an API key. Set it in environment variables.")
    ...
    # Create LLM configuration
    llm_config = LLMConfig(
        provider="gemini/gemini-1.5-pro", api_token="env:GEMINI_API_KEY"
    )
    ...
    schema = JsonCssExtractionStrategy.generate_schema(
        html=sample_html, llm_config=llm_config, query="Extract product name and price"
    )
    print("\n✅ Generated Schema:")
    pprint(schema)
