...
async def basic_llm_extraction():
    ...
    config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS, table_extraction=table_strategy
    )

    async with AsyncWebCrawler() as crawler:
        # Extract tables from a Wikipedia page
        result = await crawler.arun(
            url="https://en.wikipedia.org/wiki/List_of_chemical_elements", config=config
        )
...
async def focused_extraction():
    ...
    llm_config = LLMConfig(
        provider="openai/gpt-4.1-mini", api_token="env:OPENAI_API_KEY"
    )
    ...
    async with AsyncWebCrawler() as crawler:
        result = await crawler.arun(url=f"raw:{test_html}", config=config)
...
async def compare_strategies():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        default_strategy = DefaultTableExtraction(table_score_threshold=3, verbose=True)

        config_default = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, table_extraction=default_strategy
        )

        result_default = await crawler.arun(
            url=f"raw:{complex_html}", config=config_default
        )
        ...
        config_llm = CrawlerRunConfig(
            cache_mode=CacheMode.BYPASS, table_extraction=llm_strategy
        )

        result_llm = await crawler.arun(url=f"raw:{complex_html}", config=config_llm)
...
async def batch_extraction():
    ...
    config = CrawlerRunConfig(
        table_extraction=table_strategy, cache_mode=CacheMode.BYPASS
    )
    ...
    async with AsyncWebCrawler() as crawler:
        for url in urls:
            ...
            if result.success and result.tables:
                ...
                if result.tables:
                    all_tables.append({"url": url, "table": result.tables[0]})
