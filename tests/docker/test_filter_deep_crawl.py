...
from crawl4ai.deep_crawling import (BFSDeepCrawlStrategy,
                                    ContentRelevanceFilter, FilterChain,
                                    URLFilter, URLPatternFilter)
...
async def test_with_docker_client(
    filter_chain: list[URLFilter], max_pages: int = 20, timeout: int = 30
) -> bool:
...
async def test_with_rest_api(
    filters: list[dict[str, Any]], max_pages: int = 20, timeout: int = 30
) -> bool:
...
async def main():
    ...
    for idx, (filter_chain, max_pages, timeout) in enumerate(
        zip(filter_chain_test_cases, max_pages_, timeouts)
    ):
        docker_passed = await test_with_docker_client(
            filter_chain=filter_chain, max_pages=max_pages, timeout=timeout
        )
        ...
        [
            {
                ...
                "params": {"patterns": ["*advanced*"], "reverse": True},
            }
        ],
        [
            {
    ...
    for idx, (filters, max_pages, timeout) in enumerate(
        zip(filters_test_cases, max_pages_, timeouts)
    ):
        rest_passed = await test_with_rest_api(
            filters=filters, max_pages=max_pages, timeout=timeout
        )
