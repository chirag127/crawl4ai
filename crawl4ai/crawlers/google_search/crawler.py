...
class GoogleSearchCrawler(BaseCrawler):
    ...
    async def run(
        ...
        **kwargs,
        ...
        url = (
            f"https://www.google.com/search?q={query}&gl=sg&hl=en"
            if search_type == "text"
            else f"https://www.google.com/search?q={query}&gl=sg&hl=en&tbs=qdr:d&udm=2"
        )
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            if search_type == "image":
                if result.js_execution_result.get("success", False) is False:
                    return json.dumps(
                        {
                            "error": result.js_execution_result.get(
                                "error", "Unknown error"
                            )
                        }
                    )
    ...
    async def _build_schemas(
        self, html: str, schema_cache_path: str = None
    ) -> Dict[str, Dict]:
