...
def simulate_crawler_task(monitor, task_id, url, simulate_failure=False):
    ...
    if (
        simulate_failure and random.random() < 0.8
    ):  # 80% chance of failure if simulate_failure is True
        monitor.update_task(
            task_id=task_id,
...
def update_queue_stats(monitor, num_queued_tasks):
    ...
    while monitor.is_running:
        queued_tasks = [
            task
            for task_id, task in monitor.get_all_task_stats().items()
...
def test_crawler_monitor():
    ...
    queue_stats_thread = threading.Thread(
        target=update_queue_stats, args=(monitor, total_urls)
    )
