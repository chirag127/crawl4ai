...
class ConfidenceTestHarness:
    ...
    async def test_confidence_progression(self):
        ...
        async with AsyncWebCrawler() as crawler:
            for i, url in enumerate(self.test_urls, 1):
                ...
                print(
                    f"  New terms added: {state.new_terms_history[-1] if state.new_terms_history else 0}"
                )
    ...
    def _debug_coverage_calculation(self, state: CrawlState, query_terms: List[str]):
        ...
        for term in query_terms:
            ...
            if df > 0:
                ...
                print(
                    f"    '{term}': doc_cov={doc_coverage:.2f}, idf={idf:.2f}, boost={1 + 0.1 * math.log1p(tf_boost):.2f} → score={term_score:.3f}"
                )
        ...
        print(
            f"    Total: {coverage_score:.3f} / {max_possible_score:.3f} = {coverage_score/max_possible_score if max_possible_score > 0 else 0:.3f}"
        )
    ...
    def _calculate_coverage_new(
        self, state: CrawlState, query_terms: List[str]
    ) -> float:
        """New coverage calculation without IDF"""
        ...
        for term in query_terms:
            ...
            if df > 0:
                ...
                print(
                    f"    '{term}': doc_cov={doc_coverage:.2f}, freq_signal={freq_signal:.2f} → score={term_score:.3f}"
                )
